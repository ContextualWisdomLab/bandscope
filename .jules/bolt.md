...
## 2026-07-25 - [Optimize diagonal window operations on large SSM matrices]
**Learning:** Performing windowed operations strictly along the diagonal of large matrices (e.g. self-similarity matrices) using Python array slicing loops creates significant linear time constant overhead due to inner loop array allocation and summation.
**Action:** Use sub-matrix diagonal vectorization (`numpy.lib.stride_tricks.sliding_window_view` coupled with `np.diagonal` and `np.einsum`) to push heavy O(N*K^2) computations down to C-level NumPy operations.
