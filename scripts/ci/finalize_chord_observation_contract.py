"""Apply the reviewed chord observation padding and regression-test fixes."""

from pathlib import Path

SOURCE = Path("services/analysis-engine/src/bandscope_analysis/chords/chord_recognizer.py")
TEST = Path("services/analysis-engine/tests/test_chord_recognizer_opt.py")
SELF = Path("scripts/ci/finalize_chord_observation_contract.py")
WORKFLOW = Path(".github/workflows/finalize-chord-observation-contract.yml")

OLD_PADDING = """        # Pad or truncate RMS to match n_frames
...
NEW_PADDING = """        # Pad or truncate RMS with a neutral value that does not imply silence.
        rms_vals = (
            rms[:n_frames]
            if len(rms) >= n_frames
...
TEST_CONTENT = '''"""Tests for edge cases in the chord recognizer observation probability building."""
...
def main() -> int:
    """Patch reviewed source and tests, then remove one-shot artifacts."""
    source = SOURCE.read_text(encoding="utf-8")
    if NEW_PADDING not in source:
        if OLD_PADDING not in source:
            raise RuntimeError("expected chord observation padding block was not found")
        source = source.replace(OLD_PADDING, NEW_PADDING, 1)
