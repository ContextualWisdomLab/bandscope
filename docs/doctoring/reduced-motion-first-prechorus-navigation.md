# Reduced-motion first-pre-chorus navigation

Workspace map navigation for tonight's first pre-chorus follows the operating-system reduced-motion preference.

When `prefers-reduced-motion: reduce` matches, `FirstPreChorusCallout` scrolls the renderer-owned song-structure section with `behavior: "auto"`. Otherwise it uses `behavior: "smooth"`.

This is a presentation contract only. Pre-chorus resolution, action-mode authority, and analysis-id isolation stay unchanged.
