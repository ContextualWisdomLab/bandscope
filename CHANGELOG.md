...
- Import a bounded, validated BandScope metadata-handoff JSON file, require an explicit fresh local-audio pairing, and reuse the handoff's focused rehearsal roles for reanalysis (#739). Competing local-audio, handoff, YouTube, and analysis actions remain locked while a local audio selection is pending, so the user's next source choice cannot be overwritten by a late async result.
