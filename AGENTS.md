...
- Name tonight's first harmonic explanation with the owning part when an active role is corroborated, the owned `harmonicExplanation` copy, the labeled section, and the time so the next action is obvious. Do not invent that copy from groove, cue, setup, simplification, overlap, range, chord labels, function labels, confirmed overrides, or confidence notes.
