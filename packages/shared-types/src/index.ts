...
/** Documented. */
function validateRehearsalDalSegno(value: unknown, path: string): string | null {
  if (!isRecord(value)) {
    return invalidField(path);
  }
...
function validateRehearsalSong(
  ...
  const extraKey = unexpectedKey(
    normalized,
    ["id", "title", "tempo", "dalSegno", "sections", "exportSummary", "collaboration", "scoreAttachments"],
    ""
  );
  if (extraKey) {
    return extraKey;
  ...
  if (normalized.dalSegno !== undefined) {
    const dalSegnoError = validateRehearsalDalSegno(normalized.dalSegno, "dalSegno");
    if (dalSegnoError) {
      return dalSegnoError;
    }
  }
