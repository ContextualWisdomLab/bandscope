...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-cutoff-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstCutoffPlanOpenAction")).toBe("{at} {role} 컷오프 열기");
      expect(t("firstCutoffPlanBody")).toBe("{at} {section}에서 {role} 파트의 컷오프 계획이 있습니다.");
      expect(t("firstCutoffPlanArmed")).toBe("{at}에서 {role} 파트의 컷오프를 맞춘 다음 합주를 시작하세요.");
    });
  });
});
