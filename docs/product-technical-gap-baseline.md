...
The current protected BandScope product source is `develop@314ddeae7b775a4957594b599358c8255617eb2e`. The recorded protection contract contains 14 required contexts and still includes `Analyze (javascript-typescript)` and `Analyze (python)`. Fresh current-head evidence shows those contexts are produced by GitHub-managed dynamic CodeQL and can be GREEN while the separate central compatibility lifecycle fails closed. Issue #1172 therefore owns reconciliation/documentation of the dynamic required-context authority with the central compatibility layer; it must not assume that the `Analyze (...)` contexts are retired merely because the repository-local scanner was removed. The current central integration vehicle is `.github#2352`; its exact producer completed exact-source CodeQL analysis and the Medium+ SARIF gate, then failed only at GHAS base/head configuration-identity proof because the target `code-scanning/analyses` endpoint returned HTTP 403. Credential selection remains `.github#2275` ownership and the actual target-repository permission/canary remains `.github#2276`. This is central control-plane evidence, not a BandScope source finding. Restoring a duplicate repository scanner, synthesizing statuses, or weakening/removing CodeQL coverage is not an acceptable workaround.
...
Active work is Draft/unshipped until normally integrated into protected `develop` with current-head gates and qualifying independent review. Exact mutable heads below are intentionally delegated to the named live PR/Issue rather than copied into this source.
...
| Crash-safe project | Issue #962; PR #970 | #970 is the canonical Project Persistence Draft. It now owns native writer admission/CAS, restart/reopen exact-content binding, revision-conflict preservation, score-recovery reconciliation, explicit Preserve/Recover/Discard intent, truthful generated recovery metadata, lifecycle freshness revalidation, and project/revision-scoped recovery authorization. A recovery choice is invalidated by project switch or accepted same-project durable revision change. These are unshipped Draft capabilities. It still must ordinarily reconcile protected Resource Admission and released Score Storage contracts before cross-owner recovery/application claims. Buyer recovery UI, broader fault injection, project deletion/rollback, independent approval and release evidence remain open. |
| Score Storage / score attachments | Issue #1239; foundation PR #865; owner PR #1241 | #865 owns bounded native PDF reads. #1241 owns private bounded publication, object identity, process-death recovery, restart inventory/content receipts, receipt-bound deletion, same-id ABA prevention, and successful-return durability. On current Draft source, macOS keeps the Score Storage lease through `fsync_volume_np(FULLSYNC|WAIT)` and other Unix through directory `sync_all()` before publication success returns. Windows retains staged-file synchronization but has no claimed directory-entry power-loss guarantee. Owner-native evidence already covers Unix permission denial, Windows ACL denial, Unix kernel-enforced write failure, and macOS raw `ENOSPC` capacity exhaustion; these remain Draft evidence rather than shipped acceptance. Packaged cancellation/process termination, sudden power loss, Windows directory-entry durability/capacity parity where required, old-unleased-build coexistence, Unix residual basename race, protected integration and recovery UI remain open. |
...
The canonical baseline branch must remain an ordinary descendant of current protected `develop`. PR #968 targets #1116 rather than protected `develop` directly. Every #1116 advance therefore changes #968's target tip and requires another ordinary non-force descendant on #968 that preserves its queue-owned files. The baseline deliberately avoids embedding the descendant #968 SHA because doing so would make the source self-invalidating at the moment the required adoption commit is created. The same principle applies to other moving Draft heads: their live PR state is exact-head authority while this document owns stable semantic status. Historical SHAs remain audit evidence only.
...
| `ProjectScopedScoreRecoveryAction` | explicit recovery decision bound to one active project aggregate revision | project identity, durable project revision/CAS, and current score lifecycle must all be revalidated immediately before Project Persistence mutation; a project switch or accepted same-project revision change invalidates old intent |
...
For score recovery, Project Persistence compares durable attachment ids with owner-supplied Score Storage object evidence. Recovery state is explicit: referenced-and-published, unreferenced-published candidate, or missing-referenced/broken attachment. Unreferenced storage objects are not automatically attached or deleted. Preserve/Recover/Discard is explicit buyer intent; Recover metadata uses a truthful generated recovery label when the original selected filename is not evidenced. The authorization is session-local rather than a durable capability: current score lifecycle evidence, active project identity, and the durable project revision/CAS that authorized the choice are all revalidated immediately before use, so a changed score state, project switch, or accepted same-project revision change invalidates stale intent.
...
Current #1241 owner-native fault evidence already exercises four storage-failure classes on its real public owner boundary: Unix permission denial, Windows ACL denial, Unix kernel-enforced write failure, and macOS raw `ENOSPC` on an isolated fixed-capacity filesystem. Those tests establish Draft native behavior only; they are not packaged-executable, sudden-power-loss, or protected/released evidence and do not justify a Windows directory-entry durability claim.

Cross-owner application recovery is still incomplete and must not be simulated by source copy or filesystem probing. After #865/#1241 become protected/released owner truth and #970 ordinarily reconciles them, Recover/Preserve/Discard execution must re-read the active project identity, durable project revision/CAS, Project Persistence reconciliation and fresh Score Storage receipt immediately before mutation. Recover becomes accepted only after Project Persistence CAS/durability succeeds. Discard uses only Score Storage receipt-bound deletion. A `missing_referenced_score_ids` condition remains visible broken-attachment state, not silent metadata cleanup.
...
Residual persistence/storage work includes buyer-visible recovery conflict/attachment UX, global/startup recovery policy, autosave/backup rotation, project deletion/rollback semantics, packaged process-kill/cancellation and sudden-power-loss fault injection across stage sync → publication → stage retirement → successful-return metadata durability, Windows directory-entry durability/capacity parity where required, old unleased-build coexistence, Unix final basename-race disposition, application downgrade/rollback policy, descriptor-bound higher-parent authority, and protected cross-owner integration. Owner-native Unix permission, Windows ACL, Unix kernel write-failure and macOS raw-ENOSPC evidence are already present on the current Score Storage Draft and must not be relabeled as wholly untested gaps.
...
Commercial blockers currently include #1129 (`libsndfile` LGPL runtime path), #1181 (upstream pretrained Demucs weight rights), #1180 (immutable commercially admissible model artifact), #1206 (clean-install moderate npm advisories), #1208 (production JS bundle warning), #1209 (GHAS code-scanning configuration continuity), #1210 (frontend coverage-policy mismatch), protected integration of #865/#1241/#970, score attachment Windows directory durability plus packaged cancellation/process-termination/sudden-power-loss acceptance, buyer-visible project/score recovery, rights-cleared real-audio MIR acceptance, and signed/notarized updater rollback evidence. Central `.github#2352` is the current CodeQL compatibility integration vehicle. Its producer can complete exact-source CodeQL analysis and clean Medium+ SARIF, but GHAS base/head configuration-identity proof remains fail-closed because the target analyses endpoint is unreadable with the available credential. `.github#2275` owns credential selection that proves analysis-read capability, while `.github#2276` owns the real target permission/canary. Dynamic `Analyze (...)` success, clean SARIF, or dispatch completion alone does not satisfy that central lifecycle, and BandScope must not duplicate or weaken the identity proof.
...
- Gawarecki, M. (2024, November 4). BC-breaking change: `torch.load` is being flipped to use `weights_only=True` by default in the nightlies after #137602. *PyTorch Developer Mailing List*. https://dev-discuss.pytorch.org/t/bc-breaking-change-torch-load-is-being-flipped-to-use-weights-only-true-by-default-in-the-nightlies-after137602/2573
