...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-articulation-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstArticulationPlanOpenAction")).toBe("{at} {role} 아티큘레이션 열기");
      expect(t("firstArticulationPlanBody")).toBe("{at} {section}에서 {role} 파트의 아티큘레이션 계획이 있습니다.");
      expect(t("firstArticulationPlanArmed")).toBe("{at}에서 {role} 파트의 아티큘레이션을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
