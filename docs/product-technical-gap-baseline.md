...
The canonical decoded artifact ceiling is an artifact bound, not a process-memory promise. The returned decoder array is rejected on sample count and its visible `nbytes` before any float32 normalization copy. If the decoder exposes a non-owning NumPy view, BandScope copies the admitted visible samples into a new owned canonical float32 array before returning the MIR artifact, so the artifact itself cannot keep a larger hidden backing allocation alive. This does not retroactively bound the decoder's transient backing allocation: decoder/resampler allocations made inside `librosa` before return, and downstream NumPy/PyTorch/model/accelerator allocations, must still be measured separately before setting CPU, RSS, GPU, or VRAM policy values. Canonical finiteness revalidation scans in chunks whose NumPy boolean mask is capped at 1 MiB rather than materializing a full-song mask. Sample reduction, unrealistic warm caches, or excluding expensive phases from the measurement do not satisfy this acceptance.
