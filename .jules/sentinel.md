...
## 2025-02-24 - [Path Traversal in Python API projectId]
**Vulnerability:** The Python analysis-engine API accepted `projectId` as an untrusted parameter from orchestration payloads. Without explicit path traversal validation at the Python layer, values like `../etc/passwd` could reach path construction logic, despite the Rust core implementing its own checks.
**Learning:** Even if the primary client enforces validation, any untrusted identifier traversing language boundaries or API layers must be explicitly sanitized by the receiving service before being joined into filesystem paths. The lack of validation on `projectId` in `bandscope_analysis.api` allowed directory traversal payloads into cache logic.
**Prevention:** Always validate identifiers that resolve to file paths at the boundaries of the receiving component. In this case, ensure cross-platform path separators and dot-segments (`.`, `..`) are rejected directly.
