...
- A direct reqwest `version` must use the canonical fully specified `MAJOR.MINOR.PATCH` Cargo default/caret requirement form. Wildcards, comparison operators, shortened requirements, prerelease/build metadata and otherwise unbounded requirements are not admitted; the committed lock remains the exact resolved-version authority.
...
Allowing `*`, lower-bound-only comparator requirements or shortened version requirements was rejected because a future lock refresh could select a SemVer-incompatible reqwest line without changing the manifest. Requiring an exact `=MAJOR.MINOR.PATCH` pin was also rejected as unnecessary duplication of the committed Cargo lock for this binary product. Cargo's recommended fully specified default/caret form keeps a declared minimum and SemVer-compatible upper bound, while `Cargo.lock` retains the exact resolved graph that CI builds with `--locked`.
...
- `09852ddb7f262e040cd8a28814bee73f9b4732af` adds RED coverage for wildcard `*` and lower-bound-only `>=0.13.5` reqwest requirements while retaining a bounded `0.13.5` control case.
- `106d1f63117cbebaeaefa11e2998260e3579d3be` makes reqwest version admission require one canonical fully specified three-component Cargo default/caret requirement before lock/TLS admission continues.
