...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-fill-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstFillPlanOpenAction")).toBe("{at} {role} 필인 열기");
      expect(t("firstFillPlanBody")).toBe("{at} {section}에서 {role} 파트의 필인 계획이 있습니다.");
      expect(t("firstFillPlanArmed")).toBe("{at}에서 {role} 파트의 필인을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
