...
- `apps/desktop/core/tests/project_format_v2_fixture.rs` — version-2 fixture migration, content-addressed migration receipt, idempotent current serialization, and selected-source preservation.
...
`project_document_with_migration_receipt` uses that same version-dispatch path and emits `ProjectMigrationReceipt`: source version (`None` only for legacy unversioned input), target version, SHA-256 of the exact input bytes, SHA-256 of the deterministic current serialization, and a `migrated` flag. The receipt is content-transformation evidence, not a signature or a claim that atomic publication/recovery completed. Re-admitting the canonical v3 output yields `migrated = false` and the same output digest.
...
8. **Migration evidence separation** — migration receipts contain only source/target versions and content digests. They do not include raw project JSON, paths, user identity, or a success claim for publication/recovery.
...
The deterministic migration receipt closes source/target/input/output evidence for the in-memory format transformation. The next migration durability slice must migrate a validated copy, retain the pre-migration known-good artifact until the migrated document opens successfully, bind that publication decision to the receipt, and define downgrade/application-rollback behavior. Bounded autosave, known-good backup rotation, global startup recovery discovery, accessible Restore / Compare / Discard UX, and exhaustive interruption/disk-full/power-loss fault injection also remain open under #962.
