...
- Name tonight's first blocked assignment in the mounted rehearsal workspace so the room can unblock the stuck job before the next run; the Open action moves to the uniquely named rendered map section, while todo, in-progress, ready assignments, comments, or approvals remain guidance-only instead of becoming navigation authority.
