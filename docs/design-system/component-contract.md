...
| First Groove Callout | workspace next-action pattern | `apps/desktop/src/features/workspace/FirstGrooveCallout.tsx` | Name the holding part when an active graph node corroborates it, the owned `groove` hint, the labeled section start, and the time. Do not invent a feel from `label`, cue text, `setupNote`, `simplification`, overlap warnings, or empty/whitespace grooves. Open scrolls the renderer-owned song-structure section. Keep the unavailable state guidance-only. Distinct from first-notes, first-tag, first-form-label, and first-simplification work. |
