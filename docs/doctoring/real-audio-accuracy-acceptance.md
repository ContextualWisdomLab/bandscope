...
`0.62.1` runtime reproducibly crashed inside librosa's Numba gufunc during that
case; `numba==0.63.1` completed the same path and the full Tier 1 suite. The
analysis engine therefore constrains the existing direct runtime dependency to
`numba>=0.63.1,<0.67.0`, with the complete `uv.lock` regenerated by uv. This is
a compatibility floor, not an accuracy claim or a security exception.
