...
## 2025-02-15 - Replace reduce with for-loop early exit
**Learning:** Unconditional `.reduce()` scans all array items even when a known minimum bound (like 'low' confidence) is found early.
**Action:** For absolute bounds checks, use a `for...of` loop with a conditional `break` to allow short-circuiting and transform an O(N) exhaustive operation into an O(N) early exit.
