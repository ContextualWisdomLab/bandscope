...
describe("FirstGrooveCallout Korean role copy", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it("keeps vowel-ending dynamic role names particle-safe before and after the groove action", () => {
    ...
    expect(screen.getByText("0:10 벌스에서 피아노 파트가 그루브를 맞춥니다.")).toBeTruthy();
    expect(screen.queryByText(/피아노이/)).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "0:10 피아노 그루브 위치 열기" }));

    expect(screen.getByText("0:10에서 피아노 파트와 함께 그루브를 맞추세요. 같이 타세요.")).toBeTruthy();
