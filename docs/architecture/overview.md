...
- BandScope should support cue-sheet or chart-style outputs derived from the same section and role model. The chart JSON leads with tonight's first playable-range action so a player can start from the same check the board already named.
