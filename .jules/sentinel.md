...
## 2026-07-31 - Trivy False Positive Secret in yt-dlp
**Vulnerability:** A third-party dependency (`yt-dlp`) contains a hardcoded AWS key in one of its extractors (`yt_dlp/extractor/shahid.py`), causing Trivy to fail the CI step due to a secret detection on the `AWS Access Key ID`.
**Learning:** Virtual environment contents (e.g. `.venv/lib/.../yt-dlp/.../shahid.py`) shouldn't be parsed directly or must be correctly ignored if scanner runs against the entire working directory. The `.trivyignore` was incomplete as the previous entries for `yt_dlp/extractor/shahid.py` either didn't match the virtual environment paths or missed the explicit false-positive ignoring strategy.
**Prevention:** Always add relative paths of third-party false positives (e.g. `yt_dlp/extractor/shahid.py`) and also specifically configure Trivy or the repo's ignored files appropriately. For Sentinel memory, when adding third-party false-positive secrets from Python dependencies to `.trivyignore`, use relative module paths instead of `.venv/` paths.
