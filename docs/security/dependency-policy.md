...
- `RUSTSEC-2026-0194` and `RUSTSEC-2026-0195` for `quick-xml 0.39.4` are allowed because they are inherited through the Tauri/plist and rfd/wayland-scanner transitive chain. Current compatible upstream crates do not yet allow `quick-xml >=0.41.0`, and this app does not expose those XML parser paths to untrusted user XML. These exceptions must be removed when upstream drops or patches the chain.
