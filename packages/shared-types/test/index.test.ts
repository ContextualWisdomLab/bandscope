...
describe("shared type helpers", () => {
  ...
  it("creates a rehearsal song with section and role level guidance", () => {
    ...
    expect(song.sections[0]?.roles[2]?.hookPlan).toContain("chorus hook");
  ...
  it("keeps optional hook plans aligned with Rust project loading", () => {
    for (const hookPlan of [
      "",
      "   ",
      "\uFEFF",
      "\u0085",
      ...
      const song = createDemoRehearsalSong();
      song.sections[0]!.roles[0]!.hookPlan = hookPlan;

      expect(isRehearsalSong(song)).toBe(false);
      expect(() => parseRehearsalSong(song)).toThrow("sections[0].roles[0].hookPlan");
    }

    const paddedPlan = createDemoRehearsalSong();
    paddedPlan.sections[0]!.roles[0]!.hookPlan = "\uFEFF Keep the melody \uFEFF";
  ...
  it("covers detailed validation branches", () => {
    ...
    const cases: Array<{ message: string; payload: unknown }> = [
      ...
      {
        message: "sections[0].roles[2].hookPlan",
        payload: createInvalidSong((song) => {
          song.sections[0]!.roles[2]!.hookPlan = 2 as never;
        })
      },
      {
