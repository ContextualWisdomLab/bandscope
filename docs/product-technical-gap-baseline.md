...
The canonical decoded artifact ceiling is an artifact bound, not a process-memory promise. The returned decoder array is rejected on sample count, its visible `nbytes`, and the byte count that an owned canonical float32 copy would require before that copy is allocated. The last check matters when a narrower floating decoder dtype fits the visible buffer budget but expansion to float32 would not. If the decoder exposes a non-owning NumPy view, BandScope copies the admitted visible samples into a new owned canonical float32 array before returning the MIR artifact, so the artifact itself cannot keep a larger hidden backing allocation alive. Even a policy-admitted copy can fail when the host allocator is exhausted; that condition is surfaced as the same payload-free `memory_budget_exceeded` policy rejection rather than a raw `MemoryError`. This preserves product failure semantics but does not imply that the process stayed inside a measured RSS budget. Decoder/resampler allocations made inside `librosa` before return, the temporary coexistence of decoder and canonical buffers, and downstream NumPy/PyTorch/model/accelerator allocations must still be measured separately before setting CPU, RSS, GPU, or VRAM policy values. Canonical finiteness revalidation scans in chunks whose NumPy boolean mask is capped at 1 MiB rather than materializing a full-song mask. Sample reduction, unrealistic warm caches, or excluding expensive phases from the measurement do not satisfy this acceptance.
