...
def duration_weighted_chord_recall(
    ...
    annotated time more than once. Non-finite, empty, or reversed annotation
    and estimate timing is invalid acceptance evidence and fails closed before
    clipping.

    Args:
        segments: ``(start, end, chord)`` estimates in seconds. Segment timing
            values must be finite and each end must be greater than its start.
    ...
    Raises:
        ValueError: If annotation or estimate timing is non-finite, empty, or
            reversed.
    """
