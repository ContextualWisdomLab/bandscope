...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-vamp-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstVampPlanOpenAction")).toBe("{at} {role} 뱀프 열기");
      expect(t("firstVampPlanBody")).toBe("{at} {section}에서 {role} 파트의 뱀프 계획이 있습니다.");
      expect(t("firstVampPlanArmed")).toBe("{at}에서 {role} 파트의 뱀프를 맞춘 다음 합주를 시작하세요.");
    });
  });
});
