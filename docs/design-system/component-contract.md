...
| Navigation Item | https://www.figma.com/design/zthWmqfNKUgJBECvv002Qk/Bandscope-Design-System-v1?node-id=18-559 | `apps/desktop/src/App.tsx` | Feature-local `NAV_ITEMS` button pattern; active maps to `aria-current="page"`. Import is always available. Export, Sections, Roles, Cues, and Transpose open the matching workspace surface after analysis. Stem Lab stays coming soon. |
