...
def _checkerboard_novelty(
    ...
    # Extract sliding window view to vectorize sub-matrix diagonal patch extraction.
    swv = sliding_window_view(ssm, (kernel_size, kernel_size))
    patches = np.diagonal(swv, axis1=0, axis2=1).transpose(2, 0, 1)

    # Calculate valid novelty scores via batch dot products
    num_patches = n - 2 * half
    sums = np.einsum("ijk,jk->i", patches[:num_patches], kernel)

    novelty[half : n - half] = sums
