...
- Added a deterministic schema-v1 offline support manifest boundary that accepts only bounded, allowlisted diagnostic evidence and excludes arbitrary paths, URLs, secrets, raw messages/stacks, environment values, subprocess arguments, and user content.
