...
/** Tonight's first named leftover last-dropout remaining after leftover last-dropout. */
export type FirstLeftoverLastDropoutRemaining = {
  sectionLabel: string;
...
/**
 * Return whether the selected part belongs to this leftover last-dropout remaining.
 *
 * A leftover last-dropout remaining is shown only after a leftover last-dropout
 * whose original sit-out, leftover, remaining leftover, last leftover, or
 * leftover last-dropout cohort includes that selected part, so a silent
 * always-active part is never told to count someone out.
 */
...
/**
 * Pick the first leftover last-dropout remaining a player should honor.
 *
 ...
 * own-property tacet. A leftover last-dropout return with nobody still out, a
 * leftover last-dropout, leftover last-return, remaining leftover, leftover
 * sit-out, leftover return, come-in, tacet, tutti, new dropout after remaining
 * leftover, leftover last-dropout without a later remaining return, or a
 * singleton leftover last-dropout is not a leftover last-dropout remaining.
 *
 ...
 * leftover last-dropout remaining is shown only after a leftover last-dropout
 * that includes that named part or a leftover last-dropout remaining of that
 * named part.
 */
export function firstLeftoverLastDropoutRemaining(
  song: RehearsalSong | unknown,
  activeRole: string | null = null
): FirstLeftoverLastDropoutRemaining | null {
  ...
  for (const sectionValue of song.sections) {
    ...
    if (pendingDropout) {
      ...
      if (returningDropouts.length > 0 && remainingDropouts.length > 0) {
        let remaining = remainingDropouts[0]!;
        const returning = returningDropouts[0]!;
        if (activeRole) {
          if (!selectedPartBelongs(dropout, activeRole)) {
            continue;
          }
          const selectedRemaining = remainingDropouts.find((node) => node!.roleId === activeRole);
