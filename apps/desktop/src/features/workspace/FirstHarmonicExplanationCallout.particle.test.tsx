...
describe("FirstHarmonicExplanationCallout Korean role copy", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it("keeps vowel-ending dynamic role names particle-safe before and after the explanation action", () => {
    ...
    expect(screen.getByText("0:10 벌스에서 피아노 파트의 화성 설명이 있습니다.")).toBeTruthy();
    expect(screen.queryByText(/피아노이/)).toBeNull();
    expect(screen.queryByText(/피아노가/)).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "0:10 피아노 화성 설명 위치 열기" }));

    expect(screen.getByText("0:10에서 피아노 파트의 화성 설명을 확인한 다음 합주를 시작하세요.")).toBeTruthy();
