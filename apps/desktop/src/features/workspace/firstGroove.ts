...
const MAX_GROOVE_CHARACTERS = 180;

/** Tonight's first named groove: the earliest owned feel and the part that holds it. */
export type FirstGroove = {
...
/** Format a non-negative groove time as m:ss for rehearsal copy. */
export function formatGrooveTime(totalSeconds: number): string {
...
/** Return a bounded owned groove hint, or null when the field cannot be shown. */
function ownedGrooveHint(section: RehearsalSection): string | null {
  if (!hasOwnData(section, "groove") || typeof section.groove !== "string") {
    return null;
  }
  const hint = section.groove.trim();
  if (hint.length === 0) {
    return null;
  }
  return truncateCodePoints(hint, MAX_GROOVE_CHARACTERS);
}
...
/** Resolve a groove after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstGroove(song: RehearsalSong): FirstGroove | null {
  if (
  ...
  const hint = ownedGrooveHint(section);
  if (hint === null) {
    return null;
  }
...
/** Return the first named groove, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstGroove(song: RehearsalSong): FirstGroove | null {
  try {
    return resolveSafeFirstGroove(song);
  } catch {
    return null;
  }
}
