...
/** Tonight's first labeled chorus: the earliest lift and the part that carries it. */
export type FirstChorus = {
  section: RehearsalSection;
  holdingRole: RehearsalRole | null;
  atSeconds: number;
};

/** Format a non-negative chorus time as m:ss for rehearsal copy. */
export function formatChorusTime(totalSeconds: number): string {
...
/** Return the first labeled chorus, or null when no safe lift remains. */
export function resolveFirstChorus(song: RehearsalSong): FirstChorus | null {
  if (!isRuntimeObject(song) || !isDenseRuntimeArray(song.sections)) {
    return null;
  }

  const chorusSections = song.sections
    .filter(
      (section) =>
  ...
  const section = chorusSections[0];
  if (!section) {
    return null;
  }
