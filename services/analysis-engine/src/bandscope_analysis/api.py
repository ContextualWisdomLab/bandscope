...
class RehearsalDcAlCodaPayload(TypedDict):
    """Typed trusted D.C. al Coda payload nested inside rehearsal songs."""
...
class RehearsalSong(TypedDict):
    ...
    dcAlCoda: NotRequired[RehearsalDcAlCodaPayload]
...
def run_analysis_job_updates(
    ...
    if request["sourceKind"] == "demo":
        result["dcAlCoda"] = {"label": "D.C. al Coda"}
    updates.append(
        _build_job_status(
            job_id=job_id,
            state="running",
