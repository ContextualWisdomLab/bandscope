...
| --- | --- | --- | --- |
| Audio Ingestion / Resource Admission & Decode | #781 / PR #866 | Draft canonical 100 MiB encoded admission, bounded logical EOF for parser/decoder virtual I/O after byte admission, source metadata preflight, 44.1 kHz mono canonical decode, policy-v3 native NumPy `float32`, returned-decoder sample/allocated-byte rejection before float32 normalization, rejection of non-owning decoder views that could retain larger hidden backing allocations, decoded sample/byte ceiling, chunked finiteness validation with at most a 1 MiB temporary boolean mask, YouTube parity, path-free publication identity with validated deserialization, and no-clobber crash-durability work | Decoder/resampler internal peak RSS and downstream RSS/VRAM are not bounded; direct-child cancellation is not process-tree containment; buyer-visible accessible cancellation and rights-cleared full-length real-audio cancellation/resource measurements remain open |
...
The canonical decoded artifact ceiling is an artifact bound, not a process-memory promise. The returned decoder array is rejected on sample count and its already-allocated `nbytes` before any float32 normalization copy, and a non-owning NumPy view is rejected because its small visible `nbytes` can keep a much larger backing allocation alive. This prevents a decoder-returned view from bypassing the canonical artifact byte budget through hidden retained memory. Canonical finiteness revalidation also scans in chunks whose NumPy boolean mask is capped at 1 MiB rather than materializing a full-song mask. Decoder/resampler allocations made inside `librosa` before return, and downstream NumPy/PyTorch/model/accelerator allocations, must still be measured separately before setting CPU, RSS, GPU, or VRAM policy values. Sample reduction, unrealistic warm caches, or excluding expensive phases from the measurement do not satisfy this acceptance.
