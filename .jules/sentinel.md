...
## 2025-02-09 - Backend URL Length Validation Bypass
**Vulnerability:** The Rust backend (`import_youtube_url` in `apps/desktop/src-tauri/src/main.rs`) did not independently verify the length of the YouTube URL early in the request pipeline. Although the frontend enforces `MAX_YOUTUBE_URL_LENGTH` (2000 characters) and a later parsing step (`is_supported_youtube_url`) does its own check, an attacker could directly invoke the Tauri API with an excessively long string, potentially exhausting resources before validation.
**Learning:** Relying solely on frontend validation for constraints like string length can expose the backend to resource exhaustion vulnerabilities. Also, defense-in-depth requires bounds checking at the earliest point of untrusted data ingress on the backend API.
**Prevention:** Always enforce maximum length limits explicitly on untrusted backend API inputs before processing, even if subsequent layers perform their own validation.
