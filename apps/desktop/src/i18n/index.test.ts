...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-setup-note next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstSetupNoteOpenAction")).toBe("{at} {role} 세팅 위치 열기");
      expect(t("firstSetupNoteBody")).toBe("{at} {section}에서 {role} 파트의 세팅 메모가 있습니다.");
      expect(t("firstSetupNoteArmed")).toBe("{at}에서 {role} 파트의 세팅을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
