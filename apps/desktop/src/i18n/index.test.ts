...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-hit-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstHitPlanOpenAction")).toBe("{at} {role} 히트 열기");
      expect(t("firstHitPlanBody")).toBe("{at} {section}에서 {role} 파트의 히트 계획이 있습니다.");
      expect(t("firstHitPlanArmed")).toBe("{at}에서 {role} 파트의 히트를 맞춘 다음 합주를 시작하세요.");
    });
  });
});
