...
describe("firstLeftoverLastDropoutRemainingLastReturn selected-role search", () => {
  ...
  it("does not tell leftover last-dropout remaining without leftover last-dropout remaining last-return to come in", () => {
    const seed = createDemoRehearsalSong();
    const template = seed.sections[0]!;
    const song: RehearsalSong = {
      ...seed,
    ...
    expect(firstLeftoverLastDropoutRemainingLastReturn(song, "bass-guitar")).toBeNull();
  });

  it("does not show another part's leftover last-dropout remaining last-return to a selected part that stayed active", () => {
    const seed = createDemoRehearsalSong();
    const template = seed.sections[0]!;
    const selectedRole = {
      ...template.roles[0]!,
    ...
    expect(firstLeftoverLastDropoutRemainingLastReturn(song, "always-active")).toBeNull();
  });
});
