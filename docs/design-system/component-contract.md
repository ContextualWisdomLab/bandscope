...
| First Entrance Callout | workspace next-action pattern | `apps/desktop/src/features/workspace/FirstEntranceCallout.tsx` | Name the first hearable part, section, and start time; show the Hear button only when an entrance is available, and keep the unavailable state guidance-only without a Hear button. |
