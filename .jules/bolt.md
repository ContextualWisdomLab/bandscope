...
## 2026-07-14 - Array.every callback execution overhead
**Learning:** Using `Array.isArray(response) && response.every(cb)` on very large arrays (e.g. PDF byte streams) creates significant O(N) callback execution overhead that can severely impact rendering or parsing on the main thread.
**Action:** Replace `Array.prototype.every()` with a standard `for` loop and an early `break` for primitive array type validations to avoid callback invocation overhead and process elements significantly faster.
