...
const MAX_CAPO_PLAN_CHARACTERS = 180;

/** Tonight's first capo plan: the earliest labeled section and the part that owns it. */
export type FirstCapoPlan = {
...
/** Format a non-negative capo-plan time as m:ss for rehearsal copy. */
export function formatCapoPlanTime(totalSeconds: number): string {
...
/** Return a bounded own capo plan, or null when it cannot be shown. */
function ownedCapoPlan(role: unknown): string | null {
  if (!isRuntimeObject(role) || !hasOwnData(role, "capoPlan")) {
    return null;
  }
  const capoPlan = (role as { capoPlan?: unknown }).capoPlan;
  if (typeof capoPlan !== "string") {
    return null;
  }
  const trimmed = capoPlan.trim();
  if (trimmed.length === 0 || trimmed.includes("\n") || trimmed.includes("\r")) {
    return null;
  }
  return truncateCodePoints(trimmed, MAX_CAPO_PLAN_CHARACTERS);
}
...
/** Resolve a capo plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstCapoPlan(song: RehearsalSong): FirstCapoPlan | null {
  ...
  const candidates = song.sections
    ...
    .flatMap((section) => {
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section).filter((role) => ownedCapoPlan(role) !== null)
      );
      if (!holdingRole) {
        return [];
      }
      const capoPlan = ownedCapoPlan(holdingRole);
      if (!capoPlan) {
        return [];
      }
      return [
...
/** Return the first named capo plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstCapoPlan(song: RehearsalSong): FirstCapoPlan | null {
  try {
    return resolveSafeFirstCapoPlan(song);
  } catch {
    return null;
  }
}
