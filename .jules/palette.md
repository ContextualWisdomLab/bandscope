...
**Action:** "출시 예정" 등 설명 툴팁이 필요한 비활성화된 액션 버튼의 경우, `title`을 버튼에 직접 붙이는 대신 포커스 가능한 비대화형 `span` (`<span tabIndex={0} title={...}>`)으로 버튼을 감싸 툴팁과 키보드 접근을 제공하세요. Native `<button>`을 포함하는 wrapper에는 `role="button"` 또는 중복 `aria-disabled`를 추가하지 마세요.
