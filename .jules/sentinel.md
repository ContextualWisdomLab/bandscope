...
**Prevention:** Remove `Path.expanduser()` on input paths in backend services receiving untrusted local file paths, and explicitly raise an error when literal parent-directory path components (`..`) are detected. Ensure 100% test coverage encompasses explicitly asserting this failure behavior and preserving valid filenames that merely contain two dots.
