...
## 2026-07-08 - Decorative icons next to text
**Learning:** Purely decorative SVG icons (like lucide-react icons) positioned next to descriptive text without `aria-hidden="true"` cause redundant screen reader announcements (e.g., announcing the icon and then the text separately), reducing the overall accessibility experience.
**Action:** Always add `aria-hidden="true"` to purely decorative SVG icons positioned next to descriptive text to prevent redundant screen reader announcements.
