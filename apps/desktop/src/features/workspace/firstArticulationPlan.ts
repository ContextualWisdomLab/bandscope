...
const MAX_ARTICULATION_PLAN_CHARACTERS = 180;
const SECTION_FORM_LABEL_SET = new Set<string>(SECTION_FORM_LABELS);

/** Tonight's first articulation plan: the earliest labeled section and the part that owns it. */
export type FirstArticulationPlan = {
...
/** Format a non-negative articulation-plan time as m:ss for rehearsal copy. */
export function formatArticulationPlanTime(totalSeconds: number): string {
...
/** Return a bounded own articulation plan, or null when it cannot be shown. */
function ownedArticulationPlan(role: unknown): string | null {
  if (!isRuntimeObject(role) || !hasOwnData(role, "articulationPlan")) {
    return null;
  }
  const articulationPlan = (role as { articulationPlan?: unknown }).articulationPlan;
...
/** Resolve an articulation plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstArticulationPlan(song: RehearsalSong): FirstArticulationPlan | null {
  ...
  const candidates = song.sections
    ...
    .flatMap((section) => {
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section).filter((role) => ownedArticulationPlan(role) !== null)
      );
      if (!holdingRole) {
        return [];
      }
      const articulationPlan = ownedArticulationPlan(holdingRole);
      if (!articulationPlan) {
        return [];
      }
      return [
...
/** Return the first named articulation plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstArticulationPlan(song: RehearsalSong): FirstArticulationPlan | null {
  try {
    return resolveSafeFirstArticulationPlan(song);
  } catch {
    return null;
  }
}
