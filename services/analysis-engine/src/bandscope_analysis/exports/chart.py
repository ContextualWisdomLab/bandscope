...
def _active_role_ids(section: Mapping[str, object]) -> list[str] | None:
    ...
    if not isinstance(part_graph, list):
        return None
    active_dict: dict[str, None] = {}
    for node in part_graph:
        if not isinstance(node, Mapping) or node.get("is_active") is not True:
            continue
        role_id = node.get("role_id")
        if isinstance(role_id, str) and role_id and role_id not in active_dict:
            active_dict[role_id] = None
    return list(active_dict.keys())
...
def _active_role_names(section: Mapping[str, object]) -> list[str]:
    ...
    names_dict: dict[str, None] = {}
    for role in _active_roles(section):
        name = _role_display_name(role)
        if name is not None and name not in names_dict:
...
def _section_cue(section: Mapping[str, object]) -> str:
    ...
    cues_dict: dict[str, None] = {}
    for role in _active_roles(section):
        cue = role.get("cue")
        if not isinstance(cue, Mapping):
            continue
        value = cue.get("value")
        if isinstance(value, str) and value and value not in cues_dict:
            cues_dict[value] = None
    return "; ".join(cues_dict.keys())
...
def _footer_lines(song: Mapping[str, object], sections: list[Mapping[str, object]]) -> list[str]:
    ...
    for section in sections:
        for role in _section_roles(section):
            ...
            if entry not in priorities_dict:
                priorities_dict[entry] = None
    if priorities_dict:
        lines.append("Priorities:")
        lines.extend(priorities_dict.keys())
