...
pub struct RehearsalRolePayload {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    cutoff_plan: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    cutoff_plan_source: Option<CutoffPlanSourcePayload>,
...
pub fn project_payload_from_content(content: &str) -> Result<RehearsalSongPayload, String> {
    if let Ok(parsed) = serde_json::from_str::<RehearsalSongPayload>(content) {
        return validate_cutoff_plan_provenance(parsed);
    }
    ...
    validate_cutoff_plan_provenance(parsed)
}

fn validate_cutoff_plan_provenance(
    ...
    for section in &payload.sections {
        for role in &section.roles {
            ...
            if role.cutoff_plan.as_ref().is_some_and(|cutoff_plan| {
                cutoff_plan.trim().is_empty()
                    || cutoff_plan.contains('\n')
                    || cutoff_plan.contains('\r')
            }) {
                return Err("Invalid project file format".to_string());
            }
            if role.cutoff_plan.is_none() && role.cutoff_plan_source.is_some() {
                return Err("Invalid project file format".to_string());
            }
            if role.cutoff_plan.is_some() && role.cutoff_plan_source.is_none() {
                return Err("Invalid project file format".to_string());
            }
        }
    }
