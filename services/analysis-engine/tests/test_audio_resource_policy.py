...
) -> pytest.RaisesExc[AudioResourcePolicyError]:
    """Expect a payload-free rejection for one stable reason code."""
    return pytest.raises(
        AudioResourcePolicyError,
        match=policy_rejection_message(rejection_reason),
    )
...
def test_policy_snapshot_is_versioned_and_matches_rehearsal_intake_evidence() -> None:
    ...
    audio_resource_policy = DEFAULT_AUDIO_RESOURCE_POLICY
    assert audio_resource_policy.policy_version == AUDIO_RESOURCE_POLICY_VERSION == 1
    assert (
        audio_resource_policy.max_encoded_file_bytes
        == MAX_ENCODED_FILE_BYTES
        == 100 * 1024 * 1024
    )
    assert audio_resource_policy.max_duration_seconds == MAX_DURATION_SECONDS == 15 * 60
    assert (
        audio_resource_policy.target_sampling_rate_hz
        == TARGET_SAMPLING_RATE_HZ
        == 44100
    )
    assert (
        audio_resource_policy.max_decoded_sample_count
        == MAX_DECODED_SAMPLE_COUNT
        == 15 * 60 * 44100
    )
    assert audio_resource_policy.max_decoded_memory_bytes == 15 * 60 * 44100 * 2 * 4


@pytest.mark.parametrize("file_size_bytes", [1, MAX_ENCODED_FILE_BYTES])
def test_encoded_file_bytes_accept_the_inclusive_ceiling(file_size_bytes: int) -> None:
    """Sizes at and below the encoded-byte ceiling are admitted."""
    validate_encoded_file_bytes(file_size_bytes)
...
def test_encoded_file_bytes_reject_the_next_byte(file_size_bytes: float) -> None:
    """One byte above the ceiling fails before decode."""
    with _policy_rejection("encoded_file_too_large"):
        validate_encoded_file_bytes(file_size_bytes)


def test_encoded_file_bytes_reject_empty_payloads() -> None:
    """Zero-byte files cannot skip decode-time measurement."""
    with _policy_rejection("malformed_header"):
...
def test_encoded_file_bytes_reject_malformed_sizes(file_size_bytes: object) -> None:
    """Boolean, fractional, non-finite, and non-numeric sizes fail closed."""
    with _policy_rejection("non_finite_metadata"):
        validate_encoded_file_bytes(file_size_bytes)
...
def test_duration_rejects_values_outside_the_window(
    duration_seconds: float,
    rejection_reason: str,
) -> None:
    """Too-short and too-long recordings name the next file-selection action."""
    with _policy_rejection(rejection_reason):
        validate_duration_seconds(duration_seconds)


@pytest.mark.parametrize("sampling_rate_hz", [8_000, 44_100, 48_000, 192_000])
def test_source_sampling_rate_accepts_supported_hosts(sampling_rate_hz: int) -> None:
    """Common rehearsal capture rates stay inside the policy."""
    validate_source_sampling_rate(sampling_rate_hz)


@pytest.mark.parametrize("sampling_rate_hz", [0, -1, 7_999, 192_001, 44_100.5])
def test_source_sampling_rate_rejects_unsupported_hosts(sampling_rate_hz: object) -> None:
    """Extreme and fractional rates fail before allocation."""
    with _policy_rejection("sampling_rate_unsupported"):
        validate_source_sampling_rate(sampling_rate_hz)


@pytest.mark.parametrize("sampling_rate_hz", [math.nan, True, None])
def test_source_sampling_rate_rejects_malformed_metadata(sampling_rate_hz: object) -> None:
...
def test_channel_count_accepts_mono_and_stereo(channel_count: int) -> None:
    """Mono and stereo remain the only admitted layouts."""
    validate_channel_count(channel_count)


@pytest.mark.parametrize("channel_count", [0, 3, 8, 1.5])
def test_channel_count_rejects_unsupported_layouts(channel_count: object) -> None:
    """Multichannel and fractional layouts fail closed."""
    with _policy_rejection("channel_count_unsupported"):
        validate_channel_count(channel_count)


@pytest.mark.parametrize("channel_count", [math.nan, True, None])
def test_channel_count_rejects_malformed_metadata(channel_count: object) -> None:
...
def test_decoded_mono_audio_at_the_sample_ceiling_is_admitted() -> None:
    ...
    audio_resource_policy = AudioResourcePolicy(
        **{
    ...
    decoded_audio = np.zeros(8, dtype=np.float32)
    validate_decoded_audio(decoded_audio, 8_000, audio_resource_policy)


def test_decoded_audio_rejects_empty_or_non_array_payloads() -> None:
    """Missing samples cannot skip the decoded-size revalidation."""
    with _policy_rejection("duration_too_short"):
...
def test_decoded_audio_rejects_non_finite_samples() -> None:
    ...
    decoded_audio = np.array([0.0, math.nan], dtype=np.float32)
    with _policy_rejection("malformed_header"):
        validate_decoded_audio(decoded_audio, 44_100)
...
def test_decoded_audio_rejects_non_numeric_dtypes(decoded_audio: np.ndarray) -> None:
...
def test_decoded_audio_rejects_sample_count_above_the_ceiling() -> None:
    ...
    with _policy_rejection("decoded_sample_count_exceeded"):
        validate_decoded_audio(np.zeros(5, dtype=np.float32), 8_000, audio_resource_policy)
...
def test_decoded_audio_rejects_duration_after_sample_count_passes() -> None:
    ...
    with _policy_rejection("duration_exceeded"):
        validate_decoded_audio(
...
def test_decoded_audio_rejects_memory_budget_after_layout_classification() -> None:
    ...
    with _policy_rejection("memory_budget_exceeded"):
        validate_decoded_audio(
            np.zeros((2, 400), dtype=np.float32), 8_000, audio_resource_policy
        )


def test_decoded_stereo_uses_channel_first_layout() -> None:
    """Librosa-style ``(channels, samples)`` arrays are classified as stereo."""
    decoded_audio = np.zeros((2, 8_000), dtype=np.float32)
    validate_decoded_audio(decoded_audio, 8_000)
...
def test_decoded_sample_first_layout_is_still_classified() -> None:
    ...
    decoded_audio = np.zeros((8_000, 2), dtype=np.float32)
    validate_decoded_audio(decoded_audio, 8_000)
...
def test_memory_estimate_uses_checked_arithmetic() -> None:
    ...
    with _policy_rejection("integer_overflow"):
...
def test_policy_error_carries_versioned_provenance() -> None:
    """Audit metadata records policy version and semantic rejection fields."""
    with pytest.raises(AudioResourcePolicyError) as caught_error:
        validate_duration_seconds(16 * 60)
    assert caught_error.value.rejection_reason == "duration_exceeded"
    assert caught_error.value.policy_version == AUDIO_RESOURCE_POLICY_VERSION
    assert "16" not in caught_error.value.safe_message
    assert "960" not in caught_error.value.safe_message
