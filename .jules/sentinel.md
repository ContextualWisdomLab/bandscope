...
## 2026-09-09 - Log Injection in Path Traversal Logging
**Vulnerability:** Logging unvalidated user inputs (like paths) directly using `%s` formatting without escaping allows Log Forging (Log Injection / CWE-117). Attackers could inject newline characters into the input path string to spoof log entries.
**Learning:** `strix` requirements force the exact use of `%s` with a variable named `path`. Directly assigning untrusted input to `path` before logging introduces the injection vulnerability.
**Prevention:** Always sanitize untrusted input by wrapping it in `repr()` (e.g., `path = repr(project_id)`) before logging. This safely escapes control characters and satisfies both security requirements and `strix` formatting constraints.
