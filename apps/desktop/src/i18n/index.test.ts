...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-overlap next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstOverlapOpenAction")).toBe("{at} {role} 겹침 위치 열기");
      expect(t("firstOverlapBody")).toBe("{at} {section}에서 {role} 파트가 겹칩니다.");
      expect(t("firstOverlapArmed")).toBe("{at}에서 {role} 파트 겹침을 풀으세요. 자리를 나눠 잡으세요.");
    });
  });
});
