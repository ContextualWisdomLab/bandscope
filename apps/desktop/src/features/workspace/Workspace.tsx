...
export function Workspace({ song, sourceBootstrap = null, onSongUpdate }: WorkspaceProps) {
  ...
  const namedTacet = useMemo(() => firstTacet(song, activeRole), [activeRole, song]);
  const firstTacetCopy = namedTacet
    ? fillRangeCopy(t("workspaceFirstTacetNamed"), {
        roleName: namedTacet.roleName,
        sectionLabel: namedTacet.sectionLabel
      })
    : t("workspaceFirstTacetMissing");
      ...
      <Card className="overflow-hidden border-white/10 bg-slate-950/78 text-slate-100 shadow-[0_24px_90px_rgba(0,0,0,0.34)] backdrop-blur-2xl">
        ...
        <CardContent className="space-y-6 bg-[linear-gradient(180deg,rgba(15,23,42,0.72),rgba(2,6,23,0.86))] p-5 md:p-7">
          ...
          <section
            ...
            data-testid="first-tacet"
            aria-label={t("workspaceFirstTacetTitle")}
          >
            <p className="text-xs font-black uppercase tracking-[0.24em] text-orange-200">{t("workspaceFirstTacetTitle")}</p>
            <p className="mt-2 text-sm leading-6 text-slate-100">{firstTacetCopy}</p>
          </section>
