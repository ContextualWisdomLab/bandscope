...
- The mounted workspace copy for tonight's first setup note must name the owning part when corroborated, the owned `setupNote` text, the labeled section, and the time so the next action is obvious. Open moves to the matching rendered map section. Do not invent that copy from groove, cue, simplification, overlap, range, chord labels, function labels, confirmed overrides, harmonic explanations, or confidence notes.
