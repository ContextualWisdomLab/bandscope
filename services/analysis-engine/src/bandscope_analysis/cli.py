...
def _read_bounded_job_file(path: str) -> bytes:
    ...
    if (
        ...
        logger.warning(
            "Security: Path validation failure - unpermitted namespace or authority",
            extra={"path": path},
        )
    ...
    if not stat.S_ISREG(before.st_mode):
        logger.warning(
            "Security: Path validation failure - not a regular file", extra={"path": path}
        )
    ...
    try:
        ...
        if not stat.S_ISREG(opened.st_mode):
            logger.warning(
                "Security: Path validation failure - descriptor yielded non-regular file",
                extra={"path": path},
            )
            raise OSError("opened job path is not a regular file")
        if (before.st_dev, before.st_ino) != (opened.st_dev, opened.st_ino):
            logger.warning(
                "Security: Path validation failure - detected potential TOCTOU",
                extra={"path": path},
            )
