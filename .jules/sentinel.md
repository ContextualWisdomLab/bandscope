...
**Vulnerability:** The regex mitigation for CSV formula injection in `escapeCsvField` accounted for structural characters and whitespaces but lacked explicit coverage for control characters like the NUL byte (`\x00`).
**Learning:** The NUL byte can be used by an attacker to bypass naive sanitization regex checks because some spreadsheet software ignores leading NUL bytes while still parsing subsequent formula characters.
**Prevention:** Extend regex mitigation rules to explicitly match and block leading NUL bytes (`\x00`), employing `eslint-disable-next-line no-control-regex` to cleanly permit the control character in the JS source.
