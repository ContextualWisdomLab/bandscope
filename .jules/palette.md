...
**Action:** Add a `title` attribute mirroring the `aria-label` (or providing a specific disabled reason) only when the icon-only button can receive pointer events. For `pointer-events-none` elements, use a focusable wrapper or custom tooltip instead so the explanation remains available.
...
**Action:** Do not rely on native `title` attributes for explaining disabled states on buttons with `pointer-events-none`. Instead, use a custom tooltip component or a focusable wrapper that can receive the pointer and keyboard focus.
...
**Action:** "출시 예정" 등 설명 툴팁이 필요한 비활성화된 액션 버튼의 경우, `title`을 버튼에 직접 붙이는 대신 포커스 가능한 `span` (`<span tabIndex={0} title={...}>`)으로 버튼을 감싸서 시각적 및 스크린 리더 접근성을 모두 보장해야 합니다. 상태와 활성화 의미는 내부 네이티브 버튼에 둡니다.
