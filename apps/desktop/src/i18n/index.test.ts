...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-harmonic-explanation next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstHarmonicExplanationOpenAction")).toBe("{at} {role} 화성 설명 위치 열기");
      expect(t("firstHarmonicExplanationBody")).toBe("{at} {section}에서 {role} 파트의 화성 설명이 있습니다.");
      expect(t("firstHarmonicExplanationArmed")).toBe(
        "{at}에서 {role} 파트의 화성 설명을 확인한 다음 합주를 시작하세요."
      );
    });
  });
});
