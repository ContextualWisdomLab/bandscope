...
/** Tonight's first named leftover last-dropout remaining last-return tutti. */
export type FirstLeftoverLastDropoutRemainingLastReturnTutti = {
  sectionLabel: string;
  remainingLastReturnSectionLabel: string;
 ...
 * last-dropout remaining last-return tutti keep its trustworthy display name
 * across leftover sit-out, remaining leftover, leftover last-return, leftover
 * last-dropout, leftover last-dropout remaining, leftover last-dropout
 * remaining last-return, and leftover last-dropout remaining last-return tutti.
 */
 ...
 * returning in the same leftover last-dropout remaining last-return tutti
 * decision.
 */
...
type PendingTutti = PendingRemainingDropout & {
  remainingLastReturnSectionLabel: string;
  leftoverAtLastReturnIds: string[];
};

/**
 * Return whether the selected part belongs to this leftover last-dropout
 * remaining last-return tutti.
 *
 * A leftover last-dropout remaining last-return tutti is shown only after
 * leftover last-dropout remaining last-return whose original sit-out, leftover,
 * remaining leftover, last leftover, leftover last-dropout, leftover last-dropout
 * remaining, leftover last-dropout remaining last-return, leftover at leftover
 * last-dropout remaining last-return, or leftover last-dropout remaining
 * last-return tutti includes that selected part, so a silent always-active part
 * is never told to count someone in.
 */
function selectedPartBelongs(
  pending: PendingRemainingDropout | PendingTutti,
  activeRole: string | null
): boolean {
  if (!activeRole) {
    return true;
  }
  const leftoverAtLastReturnIds =
    "leftoverAtLastReturnIds" in pending ? pending.leftoverAtLastReturnIds : [];
  return (
    ...
    leftoverAtLastReturnIds.includes(activeRole)
  );
}

/**
 * Pick the first leftover last-dropout remaining last-return tutti a player
 * should honor.
 *
 ...
 * last-dropout remaining named part is own-property active and at least one
 * named part remains own-property tacet. A leftover last-dropout remaining
 * last-return tutti is the first later named section after leftover last-dropout
 * remaining last-return where every named graph role is own-property active.
 * Leftover last-dropout remaining last-return that is already all-in, leftover
 * last-dropout remaining, leftover last-dropout, leftover last-dropout return
 * with nobody still out, leftover last-return, remaining leftover, leftover
 * sit-out, leftover return, come-in, tacet, tutti after leftover last-return
 * without leftover last-dropout remaining last-return, new dropout after
 * remaining leftover, leftover last-dropout without leftover last-dropout
 * remaining, leftover last-dropout remaining last-return without a later tutti,
 * or a singleton leftover last-dropout is not leftover last-dropout remaining
 * last-return tutti.
 *
 ...
 * leftover last-dropout remaining last-return tutti is shown only after leftover
 * last-dropout remaining last-return that includes that named part, leftover at
 * leftover last-dropout remaining last-return of that named part, or leftover
 * last-dropout remaining last-return tutti of that named part.
 */
export function firstLeftoverLastDropoutRemainingLastReturnTutti(
  song: RehearsalSong | unknown,
  activeRole: string | null = null
): FirstLeftoverLastDropoutRemainingLastReturnTutti | null {
  ...
  for (const sectionValue of song.sections) {
    ...
    if (pendingRemainingDropout) {
      ...
      if (returningLast.length > 0 && stillRemaining.length === 0) {
        ...
        if (activeRole) {
          if (!selectedPartBelongs(remaining, activeRole) && sittingOut.every((node) => node.roleId !== activeRole)) {
            continue;
          }
        ...
        pendingTutti = {
          ...
          remainingLastReturnSectionLabel: sectionLabel,
          leftoverAtLastReturnIds: sittingOut.map((node) => node.roleId)
        };
        pendingRemainingDropout = null;
        continue;
      }
