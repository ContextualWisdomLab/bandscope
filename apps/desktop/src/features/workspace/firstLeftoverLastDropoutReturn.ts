...
/** Tonight's first named leftover last-dropout return after leftover last-dropout. */
export type FirstLeftoverLastDropoutReturn = {
  sectionLabel: string;
 ...
 * last-dropout keep its trustworthy display name across leftover sit-out,
 * remaining leftover, leftover last-return, leftover last-dropout, and leftover
 * last-dropout return.
 */
 ...
 * closed so a leftover last-dropout cannot be both sitting out and returning in
 * the same leftover last-dropout return decision.
 */
...
/**
 * Return whether the selected part belongs to this leftover last-dropout return.
 *
 * A leftover last-dropout return is shown only after a leftover last-dropout
 * whose original sit-out, leftover, remaining leftover, last leftover, or
 * dropping named part includes that selected part, so a silent always-active
 * part is never told to count someone back in.
 */
function selectedPartBelongs(pending: PendingDropout, activeRole: string | null): boolean {
  if (!activeRole) {
    return true;
  }
  return (
    ...
    pending.dropoutRoleId === activeRole
  );
}

/**
 * Pick the first leftover last-dropout return a player should honor.
 *
 ...
 * leftover last-return where at least one named part is own-property tacet. A
 * leftover last-dropout return is the first later named section after that
 * leftover last-dropout where that leftover last-dropout named part is
 * own-property active. A leftover last-dropout, leftover last-return,
 * remaining leftover, leftover sit-out, leftover return, come-in, tacet,
 * tutti, new dropout after remaining leftover, or leftover last-dropout
 * without a later return is not a leftover last-dropout return.
 *
 ...
 * leftover last-dropout return is shown only after a leftover last-dropout
 * that includes that named part or a later return of that named part.
 */
export function firstLeftoverLastDropoutReturn(
  song: RehearsalSong | unknown,
  activeRole: string | null = null
): FirstLeftoverLastDropoutReturn | null {
  ...
  for (const sectionValue of song.sections) {
    ...
    if (pendingDropout) {
      ...
      const dropoutNode = nodes.find((node) => node.roleId === dropout.dropoutRoleId);
      if (!dropoutNode) {
        return null;
      }
      if (dropoutNode.active === false) {
        continue;
      }
      if (!selectedPartBelongs(dropout, activeRole)) {
        continue;
      }
      return {
        sectionLabel,
        ...
        dropoutRoleId: dropout.dropoutRoleId,
        dropoutRoleName: namedRoles.get(dropout.dropoutRoleId)!
      };
    }
