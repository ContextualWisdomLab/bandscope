...
const MAX_PAD_PLAN_CHARACTERS = 180;
...
/** Tonight's first pad plan: the earliest labeled section and the part that owns it. */
export type FirstPadPlan = {
...
/** Format a non-negative pad-plan time as m:ss for rehearsal copy. */
export function formatPadPlanTime(totalSeconds: number): string {
...
/** Return a bounded snapshotted own pad plan, or null when it cannot be shown. */
function ownedPadPlan(role: unknown): string | null {
  if (!isRuntimeObject(role)) {
    return null;
  }
  const padPlan = ownDataValue(role, "padPlan");
  if (typeof padPlan !== "string") {
    return null;
  }
  if (!isNonEmptySingleLineText(padPlan)) {
    return null;
  }
  const trimmed = padPlan.trim();
  return truncateCodePoints(trimmed, MAX_PAD_PLAN_CHARACTERS);
}
...
/** Resolve a pad plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstPadPlan(song: RehearsalSong): FirstPadPlan | null {
    ...
    .flatMap((section, sectionIndex) => {
      ...
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section as RehearsalSection).filter(
          (metadata) => ownedPadPlan(metadata.role) !== null
        )
      );
      if (!holdingRole) {
        return [];
      }
      const padPlan = ownedPadPlan(holdingRole.role);
      if (!padPlan) {
        return [];
      }
      return [
...
/** Return the first named pad plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstPadPlan(song: RehearsalSong): FirstPadPlan | null {
  try {
    return resolveSafeFirstPadPlan(song);
  } catch {
    return null;
  }
}
