...
describe("shared type helpers", () => {
  ...
  it("creates a rehearsal song with section and role level guidance", () => {
    ...
    expect(song.sections[0]?.roles[0]?.dynamicsPlan).toContain("chorus still has somewhere to lift");
  ...
  it("keeps optional dynamics plans aligned with Rust project loading", () => {
    for (const dynamicsPlan of [
      "",
      "   ",
      "\uFEFF",
      "\u0085",
      ...
      const song = createDemoRehearsalSong();
      song.sections[0]!.roles[0]!.dynamicsPlan = dynamicsPlan;

      expect(isRehearsalSong(song)).toBe(false);
      expect(() => parseRehearsalSong(song)).toThrow("sections[0].roles[0].dynamicsPlan");
    }

    const paddedPlan = createDemoRehearsalSong();
    paddedPlan.sections[0]!.roles[0]!.dynamicsPlan = "\uFEFF Hold the string \uFEFF";
  ...
  it("covers detailed validation branches", () => {
    ...
    const cases: Array<{ message: string; payload: unknown }> = [
      ...
      {
        message: "sections[0].roles[0].dynamicsPlan",
        payload: createInvalidSong((song) => {
          song.sections[0]!.roles[0]!.dynamicsPlan = 2 as never;
        })
      },
      {
