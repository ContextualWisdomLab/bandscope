//! Streaming SHA-256 used to bind native media bytes to validated metadata.
//!
//! BandScope uses this implementation only for deterministic local integrity
//! verification. It follows the SHA-256 operations specified by NIST FIPS 180-4
//! and is exercised against published-style known-answer vectors; those tests do
//! not constitute CAVP validation.
...
impl Sha256State {
    fn update(&mut self, mut bytes: &[u8]) {
        self.message_len_bytes = self
            ...
            .expect("BandScope SHA-256 input length must fit the FIPS 180-4 length field");
        ...
        while bytes.len() >= BLOCK_BYTES {
            let block: &[u8; BLOCK_BYTES] = bytes[..BLOCK_BYTES]
                .try_into()
                .expect("SHA-256 block slice has fixed length");
            self.compress(block);
    ...
    fn finalize(mut self) -> [u8; DIGEST_BYTES] {
        let message_len_bits = self
            .message_len_bytes
            .checked_mul(8)
            .expect("BandScope SHA-256 input length must fit the FIPS 180-4 bit length field");
    ...
    fn compress(&mut self, block: &[u8; BLOCK_BYTES]) {
        ...
        for (index, chunk) in block.chunks_exact(4).enumerate() {
            schedule[index] = u32::from_be_bytes(
                chunk
                    .try_into()
                    .expect("SHA-256 message word has exactly four bytes"),
            );
        }
        for index in 16..64 {
...
/// Read all bytes from `reader` and return their lowercase SHA-256 digest.
///
/// The function streams fixed-size chunks instead of buffering media files, so
/// verifying a multi-gigabyte classic RIFF/WAV artifact does not scale heap use
/// with file size.
...
mod tests {
    ...
    impl Read for ShortReader {
        fn read(&mut self, output: &mut [u8]) -> io::Result<usize> {
            ...
            let copied = remaining.min(self.max_chunk).min(output.len());
    ...
    #[test]
    fn streaming_short_reads_do_not_change_the_digest() {
        let bytes = (0..131_111).map(|index| (index % 251) as u8).collect::<Vec<_>>();
        let expected = sha256_hex_reader(Cursor::new(&bytes)).expect("reference digest should hash");
        let actual = sha256_hex_reader(ShortReader {
            bytes,
            cursor: 0,
            max_chunk: 7,
        })
        .expect("short-read digest should hash");

        assert_eq!(actual, expected);
    }

    #[test]
    fn matches_the_million_a_sha256_vector() {
        let bytes = vec![b'a'; 1_000_000];
        assert_eq!(
            sha256_hex_reader(Cursor::new(bytes)).as_deref(),
            Ok("cdc76e5c9914fb9281a1c7e284d73e67f1809a48a497200e046d39ccc7112cd0")
        );
    }
}
