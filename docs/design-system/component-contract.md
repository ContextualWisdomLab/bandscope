...
| Metric Card | https://www.figma.com/design/zthWmqfNKUgJBECvv002Qk/Bandscope-Design-System-v1?node-id=19-216 | `apps/desktop/src/App.tsx` | Feature-local `MetricCard({ icon, label, value, detail, accent?, actionLabel?, onAction? })`. After analysis, Tempo/Key/Transpose name a countable BPM, first-entrance chord shape, and transpose setup when owned fields exist; otherwise they stay pending. Optional `actionLabel`/`onAction` scroll renderer-owned workspace surfaces. Metrics follow source controls on mobile. |
