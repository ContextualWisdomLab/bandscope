...
class TestNoPathLeakage:
    ...
    def test_deduplication_preserves_first_occurrence_order(self) -> None:
        """Deduplication uses dictionary keys to preserve insertion order exactly."""
        song = _demo_song()
        section = song["sections"][0]
        # Set up a sequence that will break an unordered set implementation
        # and test deduplication across disjoint identical values.
        section["roles"] = [
            _role("bass", "Bass", "Walk up"),
            _role("drums", "Drums", "Hit on 1"),
            _role("keys", "Keys", "Hold chord"),
            _role("bass-copy", "Bass", "Walk up"),
            _role("drums-copy", "Drums", "Hit on 1"),
            _role("keys-copy", "Keys", "Hold chord"),
        ]
        for role in section["roles"]:
            role["rehearsalPriority"] = "medium"
        section["roles"][0]["rehearsalPriority"] = "high"
        section["roles"][2]["rehearsalPriority"] = "low"

        section["partGraph"] = [
            {"role_id": "bass", "is_active": True, "handoff_to": [], "handoff_from": []},
            {"role_id": "drums", "is_active": True, "handoff_to": [], "handoff_from": []},
            {"role_id": "keys", "is_active": True, "handoff_to": [], "handoff_from": []},
            {"role_id": "bass", "is_active": True, "handoff_to": [], "handoff_from": []},
            {"role_id": "drums", "is_active": True, "handoff_to": [], "handoff_from": []},
            {"role_id": "keys", "is_active": True, "handoff_to": [], "handoff_from": []},
        ]
        ...
        rows = build_cue_sheet_rows(song)
        # Order should be exact and duplicates removed
        assert rows[0]["roles"] == ["Bass", "Drums", "Keys"]
        assert rows[0]["cue"] == "Walk up; Hit on 1; Hold chord"

        text = build_chart_text(song)
        # Priorities deduplication
        assert "  - Bass: high" in text
        assert "  - Drums: medium" in text
        assert "  - Keys: low" in text
