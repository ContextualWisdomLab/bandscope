...
const MAX_FADE_PLAN_CHARACTERS = 180;
...
const FADE_PLAN_SOLO = "Fade this part; let the next downbeat land quieter.";
const FADE_PLAN_PREFIX = "Fade this part with ";
const FADE_PLAN_SUFFIX = "; let the next downbeat land quieter.";

type FadePlanSource = "model" | "user";

/** Structured localization guidance for model-generated fade-plan copy. */
export type FadePlanGuidance =
  | Readonly<{ kind: "solo" }>
...
/** Tonight's first fade plan: the earliest labeled intensity fall on staying sources. */
export type FirstFadePlan = {
...
/** Format a non-negative fade-plan time as m:ss for rehearsal copy. */
export function formatFadePlanTime(totalSeconds: number): string {
...
/** Preserve the engine fade template while bounding its model-owned target and localization guidance. */
function boundedGeneratedFadePlan(value: string): OwnedFadePlan | null {
  if (value === FADE_PLAN_SOLO) {
    return { text: FADE_PLAN_SOLO, source: "model", guidance: { kind: "solo" } };
  }
  if (!value.startsWith(FADE_PLAN_PREFIX) || !value.endsWith(FADE_PLAN_SUFFIX)) {
    return null;
  }
  const target = value.slice(FADE_PLAN_PREFIX.length, -FADE_PLAN_SUFFIX.length);
  if (target.trim().length === 0) {
    return null;
  }
  const fixedLength = Array.from(FADE_PLAN_PREFIX + FADE_PLAN_SUFFIX).length;
  const boundedTarget = truncateCodePoints(target, MAX_FADE_PLAN_CHARACTERS - fixedLength);
  return {
    text: `${FADE_PLAN_PREFIX}${boundedTarget}${FADE_PLAN_SUFFIX}`,
    source: "model",
    guidance: { kind: "role", targetRoleName: boundedTarget }
  };
}

/** Return a bounded snapshotted own fade plan and its explicit provenance, or null when malformed. */
function ownedFadePlan(role: unknown): OwnedFadePlan | null {
  if (!isRuntimeObject(role)) {
    return null;
  }
  const fadePlan = ownDataValue(role, "fadePlan");
  const fadePlanSource = ownDataValue(role, "fadePlanSource");
  if (typeof fadePlan !== "string") {
    return null;
  }
...
function fadeSourceId(roleId: string): string {
...
function activeSourceIds(activeIds: Set<string>): Set<string> {
  return new Set([...activeIds].map((roleId) => fadeSourceId(roleId)));
}

/** Resolve a fade plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstFadePlan(song: RehearsalSong): FirstFadePlan | null {
    ...
    .flatMap((section, sectionIndex) => {
      ...
      const landingRole = pickLandingRole(
        rankedActiveRoles(section as RehearsalSection).flatMap((metadata) => {
          ...
          const fadePlan = ownedFadePlan(metadata.role);
          return fadePlan === null
            ? []
            : [
                {
                  ...metadata,
                  fadePlan: fadePlan.text,
                  fadePlanSource: fadePlan.source,
                  fadePlanGuidance: fadePlan.guidance
                }
              ];
        })
      );
      ...
      return [
        {
          ...
          fadePlan: landingRole.fadePlan,
          fadePlanSource: landingRole.fadePlanSource,
          fadePlanGuidance: landingRole.fadePlanGuidance,
...
/** Return the first named fade plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstFadePlan(song: RehearsalSong): FirstFadePlan | null {
  try {
    return resolveSafeFirstFadePlan(song);
  } catch {
    return null;
  }
}
