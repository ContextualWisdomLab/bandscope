...
const HARNESS_BIN_ENV: &str = "BANDSCOPE_SCORE_STORAGE_FAULT_HARNESS_BIN";
...
fn package_fault_harness() -> PathBuf {
    PathBuf::from(
        std::env::var_os(HARNESS_BIN_ENV)
            .expect("owner workflow must provide the built desktop-package fault harness"),
    )
}
