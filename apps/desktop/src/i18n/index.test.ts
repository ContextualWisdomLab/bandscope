...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-voicing-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstVoicingPlanOpenAction")).toBe("{at} {role} 보이싱 열기");
      expect(t("firstVoicingPlanBody")).toBe("{at} {section}에서 {role} 파트의 보이싱 계획이 있습니다.");
      expect(t("firstVoicingPlanArmed")).toBe("{at}에서 {role} 파트의 보이싱을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
