...
describe("GrooveMap", () => {
  ...
  it("preserves predecessor non-finite offset propagation until the shared validator owns that policy", () => {
    render(
      <GrooveMap
        notes={[
          { pitch: "D2", onset: 0, offset: Number.NaN, velocity: 0.5 },
          { pitch: "E2", onset: 10, offset: 20, velocity: 0.8 }
        ]}
      />
    );

    const finiteNote = screen.getByTitle("E2 (10.00s - 20.00s)");

    expect(finiteNote.style.left).toBe("");
    expect(finiteNote.style.width).toBe("");
  });
});
