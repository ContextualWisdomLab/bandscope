...
- when a decoder exposes PCM bytes for scientific consumption, independently recomputes the digest and frame count from that exact byte sequence, rejects an inconsistent decoder claim, and copies the sequence into immutable `bytes` before the callback boundary;
