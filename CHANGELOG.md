...
- Pin the canonical local-audio decoder to explicit NumPy `float32` output and `soxr_hq` band-limited resampling instead of inheriting librosa defaults, so dependency upgrades cannot silently redefine the 44.1 kHz mono waveform admitted to MIR.
