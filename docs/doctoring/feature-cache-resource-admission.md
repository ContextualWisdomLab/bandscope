...
- Persisted `separation.duration_seconds`, when present in the second-read sidecar, must convert to one finite positive real value. Python-specific `NaN`, `Infinity`, `-Infinity`, Boolean, zero, negative, and integer values too large for finite-float conversion fail closed before archive replay.
