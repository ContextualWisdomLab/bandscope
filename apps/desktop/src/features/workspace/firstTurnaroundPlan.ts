...
const MAX_TURNAROUND_PLAN_CHARACTERS = 180;
const GENERATED_ACTIVITY_TURNAROUND_PLAN_PREFIX = "Turn these last bars with ";
const GENERATED_ACTIVITY_TURNAROUND_PLAN_SUFFIX = "; land the downbeat together.";
const GENERATED_ACTIVITY_TURNAROUND_PLAN_FIXED_CHARACTERS = Array.from(
...
export type FirstTurnaroundPlan = {
...
/** Format a non-negative turnaround-plan time as m:ss for rehearsal copy. */
export function formatTurnaroundPlanTime(totalSeconds: number): string {
...
/** Keep a bounded engine-owned turnaround sentence structurally recognizable for localization. */
function boundedGeneratedActivityTurnaroundPlan(value: string): string | null {
  if (
    !value.startsWith(GENERATED_ACTIVITY_TURNAROUND_PLAN_PREFIX) ||
    !value.endsWith(GENERATED_ACTIVITY_TURNAROUND_PLAN_SUFFIX)
  ) {
    return null;
  }
  const target = value
    .slice(
      GENERATED_ACTIVITY_TURNAROUND_PLAN_PREFIX.length,
      value.length - GENERATED_ACTIVITY_TURNAROUND_PLAN_SUFFIX.length
    )
    .trim();
  if (target.length === 0) {
    return null;
  ...
  const boundedTarget = truncateCodePoints(
    target,
    MAX_TURNAROUND_PLAN_CHARACTERS - GENERATED_ACTIVITY_TURNAROUND_PLAN_FIXED_CHARACTERS
  );
...
/** Resolve a turnaround plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstTurnaroundPlan(song: RehearsalSong): FirstTurnaroundPlan | null {
...
/** Return the first named turnaround plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstTurnaroundPlan(song: RehearsalSong): FirstTurnaroundPlan | null {
  try {
    return resolveSafeFirstTurnaroundPlan(song);
  } catch {
    return null;
  }
}
