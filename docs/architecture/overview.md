...
- BandScope should support cue-sheet or chart-style outputs derived from the same section and role model. When a valid named playable range exists, the chart JSON leads with tonight's first playable-range action so a player can start from the same check the board already named; otherwise it omits `firstAction` rather than inventing guidance.
