...
- The mounted workspace copy for tonight's first harmonic explanation must name the owning part when corroborated, the owned `harmonicExplanation` text, the labeled section, and the time so the next action is obvious. Open moves to the matching rendered map section. Do not invent that copy from groove, cue, setup, simplification, overlap, range, chord labels, function labels, confirmed overrides, or confidence notes.
