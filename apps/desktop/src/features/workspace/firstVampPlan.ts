...
/** Format a non-negative vamp-plan time as m:ss for rehearsal copy. */
export function formatVampPlanTime(totalSeconds: number): string {
...
/** Resolve a vamp plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstVampPlan(song: RehearsalSong): FirstVampPlan | null {
    ...
    .flatMap((section, sectionIndex) => {
      ...
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section as RehearsalSection).filter(
          (metadata) => ownedVampPlan(metadata.role) !== null
        )
      );
      if (!holdingRole) {
        return [];
      }
      const vampPlan = ownedVampPlan(holdingRole.role);
      if (!vampPlan) {
        return [];
      }
      return [
...
/** Return the first named vamp plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstVampPlan(song: RehearsalSong): FirstVampPlan | null {
  try {
    return resolveSafeFirstVampPlan(song);
  } catch {
    return null;
  }
}
