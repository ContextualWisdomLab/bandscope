...
- Re-admit persisted stem-feature caches before MIR reuse: preflight the exact deflated NPY members, bounded headers, declared one-dimensional floating sample counts and bytes before NumPy materialization, then return only owned finite canonical float32 stems that pass the live `AudioResourcePolicy`; malformed, oversized, representation-drifted, or corrupted caches become misses rather than rehearsal evidence.
