...
- `apps/desktop` — Tauri 2 + Vite + React 19 shell (Tailwind 4, Base UI, Storybook). Feature screens live in `src/features/` (home, workspace, chords, ranges, player, settings). The mounted workspace names tonight's first harmonic explanation and opens the matching rendered map section. Do not invent that copy from groove, cue, setup, simplification, overlap, range, chord labels, function labels, confirmed overrides, or confidence notes. `src/lib/analysis.ts` and `src/lib/job_runner.ts` call typed Tauri IPC commands, with a browser fallback that serves demo data when not running inside Tauri.
