...
| First Confirmed Harmony Callout | workspace next-action pattern | `apps/desktop/src/features/workspace/FirstConfirmedHarmonyCallout.tsx` | Name the owning part when an active graph node corroborates it, the owned `manualOverrides` user-confirmed chord, the labeled section start, and the time. Do not invent a confirmed chord from `groove`, cue text, `setupNote`, `simplification`, overlap warnings, range copy, model `harmony`, or user provenance without an owned override. Open scrolls the renderer-owned song-structure section. Keep the unavailable state guidance-only. Distinct from first-ear-check, first-overlap, first-groove, first-simplification, and first-form-label work. |
