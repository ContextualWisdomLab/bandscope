...
const MAX_BREAKDOWN_PLAN_CHARACTERS = 180;
...
const BREAKDOWN_PLAN_SOLO = "Hold this breakdown; keep it sparse until the drop.";
const BREAKDOWN_PLAN_PREFIX = "Hold this breakdown with ";
const BREAKDOWN_PLAN_SUFFIX = "; keep it sparse until the drop.";

type BreakdownPlanSource = "model" | "user";

/** Structured localization guidance for model-generated breakdown-plan copy. */
export type BreakdownPlanGuidance =
  | Readonly<{ kind: "solo" }>
...
/** Tonight's first breakdown plan: the earliest labeled density drop a staying part holds. */
export type FirstBreakdownPlan = {
  ...
  breakdownPlanGuidance: BreakdownPlanGuidance | null;
  atSeconds: number;
};

/** Format a non-negative breakdown-plan time as m:ss for rehearsal copy. */
export function formatBreakdownPlanTime(totalSeconds: number): string {
...
function boundedGeneratedBreakdownPlan(value: string): OwnedBreakdownPlan | null {
  ...
  if (!value.startsWith(BREAKDOWN_PLAN_PREFIX) || !value.endsWith(BREAKDOWN_PLAN_SUFFIX)) {
    return null;
  }
  const target = value.slice(BREAKDOWN_PLAN_PREFIX.length, -BREAKDOWN_PLAN_SUFFIX.length);
  if (target.trim().length === 0) {
    return null;
  }
  const fixedLength = Array.from(BREAKDOWN_PLAN_PREFIX + BREAKDOWN_PLAN_SUFFIX).length;
  const boundedTarget = truncateCodePoints(target, MAX_BREAKDOWN_PLAN_CHARACTERS - fixedLength);
  return {
    text: `${BREAKDOWN_PLAN_PREFIX}${boundedTarget}${BREAKDOWN_PLAN_SUFFIX}`,
    source: "model",
    guidance: { kind: "role", targetRoleName: boundedTarget }
  };
}

/** Return a bounded snapshotted own breakdown plan and its explicit provenance, or null when malformed. */
function ownedBreakdownPlan(role: unknown): OwnedBreakdownPlan | null {
  if (!isRuntimeObject(role)) {
    return null;
  }
  const breakdownPlan = ownDataValue(role, "breakdownPlan");
  const breakdownPlanSource = ownDataValue(role, "breakdownPlanSource");
  if (typeof breakdownPlan !== "string") {
    return null;
  }
  if (
    breakdownPlanSource !== undefined &&
...
function breakdownSourceId(roleId: string): string {
...
function pickHoldingRole<Role extends RankedRoleMetadata>(roles: Role[]): Role | null {
  if (roles.length === 0) {
    return null;
  }
  return (
...
function activeSourceCount(activeIds: Set<string>): number {
  return new Set([...activeIds].map((roleId) => breakdownSourceId(roleId))).size;
}

/** Resolve a breakdown plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstBreakdownPlan(song: RehearsalSong): FirstBreakdownPlan | null {
  ...
  const candidates = sections
    .flatMap((section, sectionIndex) => {
      ...
      if (previousSourceCount < 3 || currentSourceCount < 1 || currentSourceCount > 2) {
        return [];
      }
      ...
      return [
        {
          ...
          breakdownPlan: holdingRole.breakdownPlan,
          breakdownPlanSource: holdingRole.breakdownPlanSource,
          breakdownPlanGuidance: holdingRole.breakdownPlanGuidance,
...
/** Return the first named breakdown plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstBreakdownPlan(song: RehearsalSong): FirstBreakdownPlan | null {
  try {
    return resolveSafeFirstBreakdownPlan(song);
  } catch {
    return null;
  }
}
