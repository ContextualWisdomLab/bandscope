...
describe("shared type helpers", () => {
  ...
  it("creates a rehearsal song with section and role level guidance", () => {
    ...
    expect(song.sections[0]?.roles[0]?.articulationPlan).toContain("longer sustain");
  ...
  it("keeps optional articulation plans aligned with Rust project loading", () => {
    for (const articulationPlan of [
      "",
      "   ",
      "\uFEFF",
      "\u0085",
      "play here\nthen move",
      "play here\rthen move",
      "play here\u0085then move"
    ]) {
      const song = createDemoRehearsalSong();
      song.sections[0]!.roles[0]!.articulationPlan = articulationPlan;

      expect(isRehearsalSong(song)).toBe(false);
      expect(() => parseRehearsalSong(song)).toThrow("sections[0].roles[0].articulationPlan");
    }

    const paddedPlan = createDemoRehearsalSong();
    paddedPlan.sections[0]!.roles[0]!.articulationPlan = "\uFEFF Play the string \uFEFF";
  ...
  it("covers detailed validation branches", () => {
    ...
    const cases: Array<{ message: string; payload: unknown }> = [
      ...
      {
        message: "sections[0].roles[0].articulationPlan",
        payload: createInvalidSong((song) => {
          song.sections[0]!.roles[0]!.articulationPlan = 2 as never;
        })
      },
      {
