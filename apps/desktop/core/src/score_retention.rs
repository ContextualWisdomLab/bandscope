...
///
/// `Ok(None)` is returned only when `symlink_metadata` observes that the exact
/// app-owned `<score_id>.pdf` directory entry does not exist. Once an entry is
...
///
/// Security Notes: the score id is allowlisted before the path join. Errors do
/// not expose the local path or score bytes. There is an unavoidable race after
/// an observed `NotFound`: another local actor can create the name before the
/// caller updates metadata, so this contract proves an absence observation, not
/// durable pathname non-existence.
