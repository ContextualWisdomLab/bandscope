...
describe("Workspace", () => {
  ...
  it("names tonight's first leftover last-dropout remaining after leftover last-dropout", () => {
    setNavigatorLanguage("en-US");
    const song = createDemoRehearsalSong();
    ...
    const callout = screen.getByTestId("first-leftover-last-dropout-remaining");
    expect(callout).toHaveTextContent("Tonight's first leftover last-dropout remaining");
    expect(callout).toHaveTextContent(
      "Bass Guitar stays out at stop while Keyboard 1 Right Hand comes back from leftover last-dropout at tag. Count Bass Guitar out from the top of stop."
    );
  });

  it("tells the leftover last-dropout remaining to stay out after leftover last-dropout", () => {
    setNavigatorLanguage("en-US");
    const song = createDemoRehearsalSong();
    const verse = song.sections[0]!;
    song.sections = [
    ...
    expect(screen.getByTestId("first-leftover-last-dropout-remaining")).toHaveTextContent(
      "Bass Guitar stays out at stop while Keyboard 1 Right Hand comes back from leftover last-dropout at tag. Stay out from the top of stop."
    );
  });

  it("asks the player to confirm leftover last-dropout remaining when every part stays active", () => {
    setNavigatorLanguage("en-US");
    const song = createDemoRehearsalSong();
    ...
    expect(screen.getByTestId("first-leftover-last-dropout-remaining")).toHaveTextContent(
      "Tonight's first leftover last-dropout remaining still needs a named leftover last-dropout cohort and a later leftover last-dropout return that leaves someone out. Confirm who stays out after leftover last-dropout before the first section."
    );
  });
