...
- Pointer hover and keyboard focus reveal the unavailable pagination explanation. The tooltip accepts pointer hit testing so moving onto the explanation does not make it disappear.
...
`apps/desktop/src/features/score/ScoreViewer.disabled-navigation-accessibility.test.tsx` verifies both ends of a three-page document. The unavailable Previous action on page 1 and unavailable Next action on page 3 each resolve `aria-describedby` to the reason-specific localized `role="tooltip"`, omit a competing unavailable-state native title, remain focusable, expose focus/hover visibility classes, and keep the tooltip pointer-hoverable. The page-1 contract also presses `Escape`, requires the tooltip and description reference to disappear without navigation, then re-focuses the same control and requires the valid reason to return.

The RED→fix evidence for the latest WCAG 1.4.13 repairs is intentionally split: `8e0012d46cc0603a836119e47cc191f462c7dc1b` required a hoverable tooltip before `78a5e60a1de7a259c45798959ca497456994fb2f` removed `pointer-events-none`; `b13c3859712f94cb66bbbbe24440f7682b3c47e7` then required Escape dismissal before `1a92b71f168a6bec64ad75de2f64f3a6fef4afa5` added the dismiss-and-retrigger behavior.
