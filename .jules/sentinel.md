...
## 2025-09-15 - Fix Log Injection via Path Formatting in Temporal Analyzer
**Vulnerability:** Untrusted user input (`path_str`) was being logged directly using f-strings in `services/analysis-engine/src/bandscope_analysis/temporal/analyzer.py`, which could allow malicious users to inject fake log entries via newline characters (Log Forging / Log Injection, CWE-117).
**Learning:** Logging arbitrary, user-controlled strings (like file paths) using f-strings without sanitization exposes the application to log forging vulnerabilities, where control characters like `\n` can span multiple log lines and falsify records.
**Prevention:** Always sanitize untrusted input before logging by wrapping it in `repr()` (e.g., `repr(path_str)`) to securely escape control characters, and use deferred string interpolation (`%s`) instead of f-strings for logging best practices.
