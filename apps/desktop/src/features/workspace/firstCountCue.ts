...
const MAX_COUNT_CUE_CHARACTERS = 180;

/** Tonight's first named count: the earliest owned entrance count and the part that carries it. */
export type FirstCountCue = {
...
/** Format a non-negative count time as m:ss for rehearsal copy. */
export function formatCountCueTime(totalSeconds: number): string {
...
/** Return a bounded owned count cue, or null when the field cannot be shown. */
function ownedCountHint(role: RehearsalRole): string | null {
  if (!hasOwnData(role, "cue")) {
    return null;
  }
  const cue = role.cue as Partial<CueAnchor> | null;
  if (
    !isRuntimeObject(cue) ||
    !hasOwnData(cue, "kind") ||
  ...
  const hint = cue.value.trim();
  if (hint.length === 0) {
    return null;
  }
  return truncateCodePoints(hint, MAX_COUNT_CUE_CHARACTERS);
}
...
/** Return the first owned count string on a section, preferring the holding part. */
function sectionCountHint(section: RehearsalSection, holdingRole: RehearsalRole | null): string | null {
  if (holdingRole) {
    const fromHolder = ownedCountHint(holdingRole);
    if (fromHolder !== null) {
      return fromHolder;
    }
  }
  ...
  for (const role of section.roles) {
    ...
    const hint = ownedCountHint(role);
    if (hint !== null) {
      return hint;
    }
  }
...
/** Resolve a count after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstCountCue(song: RehearsalSong): FirstCountCue | null {
  ...
  const candidates = song.sections
    .filter(
      (section) =>
        ...
        sectionCountHint(section, null) !== null
    )
    .sort((left, right) => {
  ...
  const holdingRole = pickHighestPriorityRole(
    rankedActiveRoles(section).filter((role) => ownedCountHint(role) !== null)
  );
  const hint = sectionCountHint(section, holdingRole);
  if (hint === null) {
    return null;
  }
...
/** Return the first named count, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstCountCue(song: RehearsalSong): FirstCountCue | null {
  try {
    return resolveSafeFirstCountCue(song);
  } catch {
    return null;
  }
}
