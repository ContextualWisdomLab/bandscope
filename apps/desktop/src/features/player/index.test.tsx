...
describe("PlayerFeature", () => {
  it("asks the room to analyze first when no song is loaded", () => {
    ...
    expect(
      screen.getByText("Analyze tonight's song first, then hear the first chorus from this player.")
    ).toBeTruthy();
  });

  it("keeps the chorus hear action unavailable without a player playback callback", () => {
    render(<PlayerFeature title="Player" song={songWithChorus()} />);

    expect(screen.queryByRole("button", { name: "Hear Lead Vocal lift at 0:30" })).toBeNull();
    expect(screen.getByText("Lead Vocal lifts the chorus at 0:30.")).toBeTruthy();
  });

  it("delegates the chorus hear action to the owning player callback", () => {
    const onPlayFromSeconds = vi.fn();
    render(<PlayerFeature title="Player" song={songWithChorus()} onPlayFromSeconds={onPlayFromSeconds} />);
  ...
  it("renders a safe empty summary when the runtime section collection is not an array", () => {
    ...
    expect(screen.getByText("No chorus yet. Stay on tonight's map until the lift is labeled.")).toBeTruthy();
