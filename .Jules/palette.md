...
## 2026-08-01 - Avoid duplicate accessibility targets with wrappers\n**Learning:** Do not wrap a disabled button inside a focusable `<span>` (`tabIndex={0}`) with `.sr-only` text to provide tooltips. This is an accessibility anti-pattern that introduces unnecessary DOM bloat and creates duplicate accessibility targets. Screen readers may announce both the focusable wrapper and the inner button, leading to a confusing auditory experience.\n**Action:** For disabled buttons, use proper aria states (`aria-disabled`) and rely on proper Tooltip components rather than creating ad-hoc wrappers with duplicate sr-only text.
