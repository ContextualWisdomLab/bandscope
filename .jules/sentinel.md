...
## 2026-08-09 - PDF.js Arbitrary JavaScript Execution Mitigation
**Vulnerability:** A vulnerability in `pdfjs-dist` (CVE-2026-16633) is flagged by Trivy, but a safe update is constrained by downstream packaging and peer dependency logic for `pdfjs-dist`, causing CI check suite failures in unrelated PRs.
**Learning:** Automatically attempting to upgrade deep or highly constrained transitive dependencies (like `pdfjs-dist`) during unrelated code health tasks (e.g. replacing `typing.Dict`) introduces unacceptable risk of application breakage, as it conflates distinct areas of concern.
**Prevention:** Suppress the CVE via `.trivyignore` to unblock CI for the isolated code-health task, ensuring the PR remains laser-focused and safely committable. Address the dependency upgrade in a dedicated, subsequent PR where the frontend integration can be thoroughly tested.
