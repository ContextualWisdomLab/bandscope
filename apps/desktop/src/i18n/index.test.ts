...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-solo-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstSoloPlanOpenAction")).toBe("{at} {role} 솔로 열기");
      expect(t("firstSoloPlanBody")).toBe("{at} {section}에서 {role} 파트의 솔로 계획이 있습니다.");
      expect(t("firstSoloPlanArmed")).toBe("{at}에서 {role} 파트의 솔로를 맞춘 다음 합주를 시작하세요.");
    });
  });
});
