...
The transport API intentionally contains no socket/client, installer, freshness repository, project persistence dependency, or verified-artifact type. The eventual HTTP adapter must be added separately with the repository's reviewed dependency admission and exact standalone lock graph.
