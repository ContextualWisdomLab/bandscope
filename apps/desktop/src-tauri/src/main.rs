...
/// same-project staging file. After the stage is synchronized, Project Persistence
/// publishes it through the platform-native no-replace owner: Linux/macOS also
/// synchronize the parent directory before success, while Windows uses the
/// write-through native move. The published object is required to remain a
...
/// a separate platform-hardening requirement; these portable checks do not claim
/// O_NOFOLLOW-equivalent race semantics.
