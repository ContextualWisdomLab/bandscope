...
const MAX_HOOK_PLAN_CHARACTERS = 180;
const SECTION_FORM_LABEL_SET = new Set<string>(SECTION_FORM_LABELS);

/** Tonight's first hook plan: the earliest labeled section and the part that owns it. */
export type FirstHookPlan = {
...
/** Format a non-negative hook-plan time as m:ss for rehearsal copy. */
export function formatHookPlanTime(totalSeconds: number): string {
...
/** Return a bounded snapshotted own hook plan, or null when it cannot be shown. */
function ownedHookPlan(role: unknown): string | null {
  if (!isRuntimeObject(role)) {
    return null;
  }
  const descriptor = Object.getOwnPropertyDescriptor(role, "hookPlan");
  if (descriptor === undefined || !Object.prototype.hasOwnProperty.call(descriptor, "value")) {
    return null;
  }
  const hookPlan = descriptor.value;
  if (typeof hookPlan !== "string") {
    return null;
  }
...
/** Resolve a hook plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstHookPlan(song: RehearsalSong): FirstHookPlan | null {
  ...
  const candidates = song.sections
    ...
    .flatMap((section) => {
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section).filter((role) => ownedHookPlan(role) !== null)
      );
      if (!holdingRole) {
        return [];
      }
      const hookPlan = ownedHookPlan(holdingRole);
      if (!hookPlan) {
        return [];
      }
      return [
...
/** Return the first named hook plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstHookPlan(song: RehearsalSong): FirstHookPlan | null {
  try {
    return resolveSafeFirstHookPlan(song);
  } catch {
    return null;
  }
}
