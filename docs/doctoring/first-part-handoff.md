...
- Tests: `firstHandoff.test.ts` and the Workspace callout cover destination labeling, activity-derived receivers that only become named in the destination section, absent/malformed destinations, inactive skip, selected-role scoping, inherited/malformed evidence, and literal copy filling.
