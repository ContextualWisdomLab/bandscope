import {
  ...
  type CollaborationAssignmentStatus,
...
const ACTIONABLE_STATUS_RANK = { in_progress: 0, todo: 1 } as const;
const MAX_ASSIGNMENT_SUMMARY_CHARACTERS = 180;

/** Tonight's first named assignment: the earliest owned actionable job and the part that carries it. */
export type FirstAssignment = {
...
/** Format a non-negative assignment time as m:ss for rehearsal copy. */
export function formatAssignmentTime(totalSeconds: number): string {
...
/** Return true when the assignment owns identity, assignee, status, and a section pointer. */
function isActionableAssignment(assignment: RehearsalAssignment): boolean {
  return (
...
/** Return the corroborated holding part, or null when the assignment is band-wide. */
...
/** Return owned sections that can host an assignment. */
...
/** Resolve an assignment after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstAssignment(song: RehearsalSong): FirstAssignment | null {
  if (
  ...
  const uniqueAssignments = uniqueOwnedById(
    song.collaboration.assignments.filter((assignment) => isActionableAssignment(assignment)),
    (assignment) =>
  ...
  const candidates = [...uniqueAssignments.values()]
    ...
    .sort((left, right) => {
      const statusDelta =
        ACTIONABLE_STATUS_RANK[left.assignment.status as Exclude<CollaborationAssignmentStatus, "ready" | "blocked">] -
        ACTIONABLE_STATUS_RANK[right.assignment.status as Exclude<CollaborationAssignmentStatus, "ready" | "blocked">];
      if (statusDelta !== 0) {
        return statusDelta;
      }
...
/** Return the first named assignment, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstAssignment(song: RehearsalSong): FirstAssignment | null {
  try {
    return resolveSafeFirstAssignment(song);
  } catch {
    return null;
  }
}
