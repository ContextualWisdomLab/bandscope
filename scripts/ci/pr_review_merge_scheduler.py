...
def is_opencode_context(node: dict[str, Any]) -> bool:
    ...
    if node.get("__typename") == "CheckRun":
        workflow = ((node.get("checkSuite") or {}).get("workflowRun") or {}).get("workflow") or {}
...
def unresolved_thread_count(pr: dict[str, Any]) -> int:
    ...
    return sum(
        1 for thread in threads if not thread.get("isResolved") and not thread.get("isOutdated")
    )
    ...
    *,
    ...
    if has_current_head_approval(pr):
        ...
        if not enable_auto_merge_flag:
            return Decision(
                number, "wait", "current head is approved; auto-merge disabled by scheduler inputs"
            )
