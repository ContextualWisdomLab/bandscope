...
env GH_TOKEN="${GH_TOKEN:-}" gh api graphql \
	-f owner="$owner" \
	-f name="$repo" \
	-F number="$PR_NUMBER" \
	-f query='
	...
	env GH_TOKEN="${GH_TOKEN:-}" gh run list \
		--repo "$GH_REPOSITORY" \
		--commit "$HEAD_SHA" \
		--limit 100 \
...

{
	...
	while IFS=$'\t' read -r kind label conclusion details_url run_id check_run_id; do
		...
		printf '\n'
			...
			if env GH_TOKEN="${GH_TOKEN:-}" gh run view "$run_id" --repo "$GH_REPOSITORY" --log-failed >"$log_file" 2>&1; then
		...
		job_json="$(mktemp)"
		tmp_files+=("$job_json")
		if env GH_TOKEN="${GH_TOKEN:-}" gh api -X GET "repos/${GH_REPOSITORY}/actions/jobs/${check_run_id}" >"$job_json" 2>/dev/null; then
			failed_steps="$(
				jq -r '
					(.steps // [])
		...
		if env GH_TOKEN="${GH_TOKEN:-}" gh api -X GET "repos/${GH_REPOSITORY}/check-runs/${check_run_id}/annotations" --paginate \
			--jq '
				.[]?
		...
		if [ -n "$run_id" ] && env GH_TOKEN="${GH_TOKEN:-}" gh run view "$run_id" \
			--repo "$GH_REPOSITORY" \
			--job "$check_run_id" \
