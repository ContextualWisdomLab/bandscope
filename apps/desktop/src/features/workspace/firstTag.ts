...
/** Tonight's first labeled tag: the earliest last-line ending and the part that holds it. */
export type FirstTag = {
  section: RehearsalSection;
  holdingRole: RehearsalRole | null;
  atSeconds: number;
};

/** Format a non-negative tag time as m:ss for rehearsal copy. */
export function formatTagTime(totalSeconds: number): string {
...
export function resolveFirstTag(song: RehearsalSong): FirstTag | null {
  ...
  const tagSections = song.sections
    .filter(
      (section) =>
