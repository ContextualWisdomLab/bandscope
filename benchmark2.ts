...
function hardcoded(currentPercent: number) {
  if (currentPercent < 20) return BROWSER_PROGRESS_STEPS[0];
  if (currentPercent < 45) return BROWSER_PROGRESS_STEPS[1];
  if (currentPercent < 70) return BROWSER_PROGRESS_STEPS[2];
  if (currentPercent < 90) return BROWSER_PROGRESS_STEPS[3];
  return undefined;
}
