...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-count next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstCountCueOpenAction")).toBe("{at} {role} 카운트 위치 열기");
      expect(t("firstCountCueBody")).toBe("{at} {section}에서 {role} 파트가 카운트합니다.");
      expect(t("firstCountCueArmed")).toBe("{at}에서 {role} 파트 카운트를 잡으세요. 함께 들어가세요.");
    });
  });
});
