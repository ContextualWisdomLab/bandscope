...
- optionally invokes an in-process `track_consumer` only after both registered audio and annotation identities have passed. The callback receives that exact canonical PCM byte view and a `memoryview` over immutable `bytes` copied from the admitted annotation snapshot, plus track ID and sample rate. It receives no workstation path and has no write capability over either scientific input;
