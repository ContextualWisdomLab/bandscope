...
fn song_with_turnaround_plan() -> Value {
    json!({
        "id": "analyzed-song",
        ...
        "sections": [{
            "id": "verse-1",
            "label": "verse",
            "groove": "Turn into the downbeat",
            "timeRange": { "start": 0, "end": 30 },
            "confidence": {
                "level": "high",
                "source": "model",
                "notes": "The turnaround is supported by the active-part transition."
            },
            "roles": [{
                "id": "bass-guitar",
                "name": "Bass Guitar",
                ...
                "cue": {
                    "kind": "transition",
                    "value": "Hold through the turnaround before the downbeat."
                },
                "range": {
                    "lowestNote": "C#2",
                    "highestNote": "E3"
...
#[test]
fn project_contract_round_trips_turnaround_plan_and_shared_role_fields() {
    let payload = song_with_turnaround_plan();
    let content = serde_json::to_string(&payload).expect("fixture should serialize");

    let parsed = project_payload_from_content(&content)
        .expect("native project contract must accept shared turnaround-plan fields");
    let serialized =
...
#[test]
fn project_contract_rejects_turnaround_plan_source_without_copy() {
    let mut payload = song_with_turnaround_plan();
    payload["sections"][0]["roles"][0]
        .as_object_mut()
        .expect("role fixture should be an object")
        .remove("turnaroundPlan");
    let content = serde_json::to_string(&payload).expect("fixture should serialize");
...
#[test]
fn project_contract_rejects_turnaround_plan_without_source() {
    let mut payload = song_with_turnaround_plan();
    payload["sections"][0]["roles"][0]
        .as_object_mut()
        .expect("role fixture should be an object")
        .remove("turnaroundPlanSource");
    let content = serde_json::to_string(&payload).expect("fixture should serialize");
...
#[test]
fn project_contract_rejects_invalid_turnaround_plan_copy() {
    for turnaround_plan in ["", "   ", "land here\nthen hold", "land here\rthen hold"] {
        let mut payload = song_with_turnaround_plan();
        payload["sections"][0]["roles"][0]["turnaroundPlan"] = json!(turnaround_plan);
        let content = serde_json::to_string(&payload).expect("fixture should serialize");
...
#[test]
fn project_contract_rejects_unknown_turnaround_plan_source() {
    let mut payload = song_with_turnaround_plan();
    payload["sections"][0]["roles"][0]["turnaroundPlanSource"] = json!("legacy");
    let content = serde_json::to_string(&payload).expect("fixture should serialize");
