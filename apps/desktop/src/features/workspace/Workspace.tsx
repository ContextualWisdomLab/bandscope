...
import { fillBarCopy, firstBarPlan } from "./firstBar";
...
export function Workspace({ song, sourceBootstrap = null, onSongUpdate }: WorkspaceProps) {
  ...
  const firstBar = useMemo(() => firstBarPlan(song), [song]);
  const firstBarCopy = firstBar
    ? fillBarCopy(
        t(firstBar.sectionLabel ? "workspaceFirstBarReady" : "workspaceFirstBarReadyNoSection"),
        {
          barLabel: firstBar.barLabel,
          sectionLabel: firstBar.sectionLabel ?? ""
        }
      )
    : t("workspaceFirstBarMissing");
      ...
      <Card className="overflow-hidden border-white/10 bg-slate-950/78 text-slate-100 shadow-[0_24px_90px_rgba(0,0,0,0.34)] backdrop-blur-2xl">
        ...
        <CardContent className="space-y-6 bg-[linear-gradient(180deg,rgba(15,23,42,0.72),rgba(2,6,23,0.86))] p-5 md:p-7">
          ...
          <section
            ...
            data-testid="first-bar"
            aria-label={t("workspaceFirstBarTitle")}
          >
            <p className="text-xs font-black uppercase tracking-[0.24em] text-sky-200">{t("workspaceFirstBarTitle")}</p>
            <p className="mt-2 text-sm leading-6 text-slate-100">{firstBarCopy}</p>
          </section>
