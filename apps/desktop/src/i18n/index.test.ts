...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-riff-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstRiffPlanOpenAction")).toBe("{at} {role} 리프 열기");
      expect(t("firstRiffPlanBody")).toBe("{at} {section}에서 {role} 파트의 리프 계획이 있습니다.");
      expect(t("firstRiffPlanArmed")).toBe("{at}에서 {role} 파트의 리프를 맞춘 다음 합주를 시작하세요.");
    });
  });
});
