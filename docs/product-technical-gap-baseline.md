...
The canonical decoded artifact ceiling is an artifact bound, not a process-memory promise. The returned decoder array is now rejected on sample count and its already-allocated `nbytes` before any float32 normalization copy, so an oversized or wide returned buffer does not trigger a second canonical allocation before rejection. Decoder/resampler allocations made inside `librosa` before return, and downstream NumPy/PyTorch/model/accelerator allocations, must still be measured separately before setting CPU, RSS, GPU, or VRAM policy values. Sample reduction, unrealistic warm caches, or excluding expensive phases from the measurement do not satisfy this acceptance.
