...
const MAX_CUTOFF_PLAN_CHARACTERS = 180;
const GENERATED_ACTIVITY_CUTOFF_PLAN_PREFIX = "Cut this off with ";
const GENERATED_ACTIVITY_CUTOFF_PLAN_SUFFIX = "; don't linger past the last beat.";
const GENERATED_ACTIVITY_CUTOFF_PLAN_FIXED_CHARACTERS = Array.from(
  GENERATED_ACTIVITY_CUTOFF_PLAN_PREFIX + GENERATED_ACTIVITY_CUTOFF_PLAN_SUFFIX
).length;
...
/** Format a non-negative cutoff-plan time as m:ss for rehearsal copy. */
export function formatCutoffPlanTime(totalSeconds: number): string {
...
/** Preserve the canonical model cutoff sentence shape while bounding its generated target text. */
function boundedGeneratedActivityCutoffPlan(value: string): string | null {
  ...
  const target = value
    .slice(
      GENERATED_ACTIVITY_CUTOFF_PLAN_PREFIX.length,
      value.length - GENERATED_ACTIVITY_CUTOFF_PLAN_SUFFIX.length
    )
    .trim();
  if (target.length === 0) {
    return null;
  ...
  const boundedTarget = truncateCodePoints(
    target,
    MAX_CUTOFF_PLAN_CHARACTERS - GENERATED_ACTIVITY_CUTOFF_PLAN_FIXED_CHARACTERS
  );
...
/** Resolve a cutoff plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstCutoffPlan(song: RehearsalSong): FirstCutoffPlan | null {
...
/** Return the first named cutoff plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstCutoffPlan(song: RehearsalSong): FirstCutoffPlan | null {
  try {
    return resolveSafeFirstCutoffPlan(song);
  } catch {
    return null;
  }
}
