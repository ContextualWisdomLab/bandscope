...
- Hardened local analysis filesystem authority so source/cache/temp inputs reject ambiguous, network/device, traversal, direct-symlink, non-directory writable-root, pre-existing derived-child, exact feature-cache file, and atomic-write temporary-file symlink escape cases before repository-owned I/O while preserving payload-safe errors and the existing missing-source result.
