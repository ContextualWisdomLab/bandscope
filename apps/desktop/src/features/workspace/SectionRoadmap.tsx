...
export function SectionRoadmap({ song, activeRole, onSongUpdate }: SectionRoadmapProps) {
  ...
  const handleChordEdit = (sectionId: string, role: RehearsalRole) => {
    ...
    if (newChord !== null && newChord.trim() !== "" && newChord !== role.harmony.chord) {
      // Performance: Replaced expensive `structuredClone` deep copy with an efficient shallow copy using object spread (`...`) and `.map()`.
      // This avoids unnecessary memory allocation overhead when updating a nested property.
      const updatedSong = {
        ...song,
