...
- The mounted workspace copy for tonight's first groove must name the holding part when corroborated, the owned feel, the labeled section, and the time so the next action is obvious. Open moves to the matching rendered map section. Do not invent a groove from label, cue, setup, simplification, or overlap copy.
