...
/** Documented. */
function validateRehearsalDcAlFine(value: unknown, path: string): string | null {
  if (!isRecord(value)) {
    return invalidField(path);
  }
  ...
  if (!/^D\.C\. al Fine(?: [1-9])?$/u.test(value.label)) {
    return invalidField(`${path}.label`);
  }
...
function validateRehearsalSong(
  ...
  const extraKey = unexpectedKey(
    normalized,
    ["id", "title", "tempo", "dcAlFine", "sections", "exportSummary", "collaboration", "scoreAttachments"],
    ""
  );
  if (extraKey) {
    return extraKey;
  ...
  if (normalized.dcAlFine !== undefined) {
    const dcAlFineError = validateRehearsalDcAlFine(normalized.dcAlFine, "dcAlFine");
    if (dcAlFineError) {
      return dcAlFineError;
    }
  }
