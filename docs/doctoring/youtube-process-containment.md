...
No hosted RED failure is claimed for `04e4d5c1…`: the regression and production repair were consecutive ordinary descendants under cancel-in-progress. Exact-head hosted evidence must come from the final descendant, not from a predecessor run. For #1244, hosted Windows run `35498831001` is the historical RED that demonstrated the namespace-coupled child re-entry failure; the final Resource Admission head requires fresh Windows and Unix evidence after the namespace-independent repair.
...
**Mitigation.** Establish containment before execution in one GUI-independent desktop-core owner, terminate the owned group before joining readers, reap the directly owned child, preserve direct-child fallback, and expose no process identifier or kill primitive to the renderer. Regression tests require the Tauri adapter to delegate to that owner and reject reintroduction of local POSIX signalling code. The large-output child regression uses a namespace-independent unique filter so Windows and Unix exercise the same branch even if Rust module layout changes.
