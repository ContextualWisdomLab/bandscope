...
const MAX_FILL_PLAN_CHARACTERS = 180;
...
/** Tonight's first fill plan: the earliest labeled section and the part that owns it. */
export type FirstFillPlan = {
...
export function formatFillPlanTime(totalSeconds: number): string {
...
function ownedFillPlan(role: unknown): string | null {
  ...
  const fillPlan = ownDataValue(role, "fillPlan");
  if (typeof fillPlan !== "string") {
    return null;
  }
  if (!isNonEmptySingleLineText(fillPlan)) {
    return null;
  }
  const trimmed = fillPlan.trim();
  return truncateCodePoints(trimmed, MAX_FILL_PLAN_CHARACTERS);
}
...
/** Resolve a fill plan after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstFillPlan(song: RehearsalSong): FirstFillPlan | null {
    ...
    .flatMap((section, sectionIndex) => {
      ...
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section as RehearsalSection).filter(
          (metadata) => ownedFillPlan(metadata.role) !== null
        )
      );
      if (!holdingRole) {
        return [];
      }
      const fillPlan = ownedFillPlan(holdingRole.role);
      if (!fillPlan) {
        return [];
      }
      return [
...
/** Return the first named fill plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstFillPlan(song: RehearsalSong): FirstFillPlan | null {
  try {
    return resolveSafeFirstFillPlan(song);
  } catch {
    return null;
  }
}
