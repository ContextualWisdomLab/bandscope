...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-breakdown-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstBreakdownPlanOpenAction")).toBe("{at} {role} 브레이크다운 열기");
      expect(t("firstBreakdownPlanBody")).toBe("{at} {section}에서 {role} 파트가 브레이크다운을 유지합니다.");
      expect(t("firstBreakdownPlanArmed")).toBe(
        "{at}에서 {role} 파트로 브레이크다운을 유지하세요. 드롭 전까지 얇게 가십시오."
      );
    });
  });
});
