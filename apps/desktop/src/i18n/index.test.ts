...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-part-handoff next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstPartHandoffOpenAction")).toBe("{at} {from} 핸드오프 위치 열기");
      expect(t("firstPartHandoffBody")).toBe("{at} {section}에서 {from} 파트가 {to} 파트로 넘깁니다.");
      expect(t("firstPartHandoffArmed")).toBe("{at}에서 {from} 파트에서 {to} 파트로 넘긴 다음 합주를 시작하세요.");
    });
  });
});
