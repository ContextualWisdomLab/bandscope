...
pub struct RehearsalRolePayload {
    ...
    setup_note: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    articulation_plan: Option<String>,
...
pub fn project_payload_from_content(content: &str) -> Result<RehearsalSongPayload, String> {
    if let Ok(parsed) = serde_json::from_str::<RehearsalSongPayload>(content) {
        return validate_articulation_plan(parsed);
    }
    ...
    validate_articulation_plan(parsed)
}
...
/// Reject blank or Unicode line-separated articulation guidance without normalizing user text.
fn is_valid_articulation_plan(value: &str) -> bool {
    ...
    for section in &payload.sections {
        for role in &section.roles {
            if role
                .articulation_plan
                .as_deref()
                .is_some_and(|articulation_plan| !is_valid_articulation_plan(articulation_plan))
            {
...
mod tests {
    ...
    fn rehearsal_song_payload_accepts_shared_section_contract() {
        ...
        assert_eq!(
            parsed.sections[0].roles[0].articulation_plan.as_deref(),
            Some(
    ...
    #[test]
    fn project_payload_from_content_rejects_invalid_articulation_plan() {
        for articulation_plan in [
            "",
            "   ",
            "\u{FEFF}",
            "\u{0085}",
            "attack here\nthen move",
            "attack here\rthen move",
            "attack here\u{0085}then move",
            "attack here\u{2028}then move",
            "attack here\u{2029}then move",
        ] {
            let mut payload = shared_contract_payload(json!({ "start": 10, "end": 30 }));
            payload["sections"][0]["roles"][0]["articulationPlan"] = json!(articulation_plan);
            let content = serde_json::to_string(&payload).expect("payload should serialize");
        ...
        let mut payload = shared_contract_payload(json!({ "start": 10, "end": 30 }));
        payload["sections"][0]["roles"][0]["articulationPlan"] =
            json!("\u{FEFF}Play the string\u{FEFF}");
