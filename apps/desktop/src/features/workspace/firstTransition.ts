...
const MAX_TRANSITION_CHARACTERS = 180;

/** Tonight's first named transition: the earliest owned change and the part that carries it. */
...
/** Return a bounded owned transition cue, or null when the field cannot be shown. */
function ownedTransitionHint(role: RehearsalRole): string | null {
  ...
  return truncateCodePoints(hint, MAX_TRANSITION_CHARACTERS);
}
...
/** Return the first owned transition string on a section, preferring the holding part. */
function sectionTransitionHint(
  section: RehearsalSection,
  holdingRole: RehearsalRole | null
): string | null {
  if (holdingRole) {
    const fromHolder = ownedTransitionHint(holdingRole);
    if (fromHolder !== null) {
      return fromHolder;
    }
  }
  ...
  for (const role of section.roles) {
    ...
    const hint = ownedTransitionHint(role);
    if (hint !== null) {
      return hint;
    }
  }
...
function resolveSafeFirstTransition(song: RehearsalSong): FirstTransition | null {
  ...
  const candidates = song.sections
    .filter(
      (section) =>
        ...
        sectionTransitionHint(section, null) !== null
    )
    .sort((left, right) => {
  ...
  const holdingRole = pickHighestPriorityRole(
    rankedActiveRoles(section).filter((role) => ownedTransitionHint(role) !== null)
  );
  const hint = sectionTransitionHint(section, holdingRole);
  if (hint === null) {
    return null;
  }
...
/** Return the first named transition, or null when untrusted runtime metadata cannot be read safely. */
