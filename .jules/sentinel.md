...
## 2026-07-03 - Backend API Project ID Traversal Guard
**Vulnerability:** The Python analysis engine API (`services/analysis-engine/src/bandscope_analysis/api.py`) lacked path traversal mitigation for `projectId`. If an attacker were able to supply a `projectId` with directory traversal segments (e.g. `../`), it could lead to files being accessed or modified outside of the intended app-managed cache or project directories when the backend joins paths.
**Learning:** Any project identifier that can reach a filesystem path join must be treated as untrusted, even when it is assumed to be internally generated. Path traversal protections must be explicitly applied at the API boundary before passing values down to path joining logic.
**Prevention:** Apply cross-platform path traversal checks (`if ".." in project_id.replace("\\", "/").split("/"):`) specifically rejecting `..` segments for `projectId` during payload normalization and validation in `validate_analysis_job_request`, ensuring traversal inputs trigger a safe failure early.
