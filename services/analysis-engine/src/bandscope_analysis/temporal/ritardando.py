"""Stamp tonight's first ritardando plan from existing tempo-stability changes.

A ritardando is the earliest sustained slowing (``to_bpm < from_bpm``) that is
not a half-time feel (~0.45–0.55) or a double-time feel (~2.0). The owned
``ritardandoPlan`` copy lands on the highest-priority active named vocal or
...
HALF_TIME_RATIO_MIN = 0.45
HALF_TIME_RATIO_MAX = 0.55
NAMED_RITARDANDO_ROLE_IDS = frozenset({"bass-guitar", "lead-vocal"})
PRIORITY_RANK = {"high": 0, "medium": 1, "low": 2}
RITARDANDO_PLAN_PREFIX = "Ease this part from "
RITARDANDO_PLAN_MIDDLE = " BPM into "
RITARDANDO_PLAN_SUFFIX = " BPM; let the next downbeat land later."


def format_ritardando_bpm(value: float) -> str | None:
...
def ritardando_plan_copy(from_bpm: float, to_bpm: float) -> str | None:
    """Return the owned model ritardando copy, or None when BPM tokens are unusable."""
    from_token = format_ritardando_bpm(from_bpm)
    to_token = format_ritardando_bpm(to_bpm)
    if from_token is None or to_token is None:
        return None
    return (
        f"{RITARDANDO_PLAN_PREFIX}{from_token}"
        f"{RITARDANDO_PLAN_MIDDLE}{to_token}{RITARDANDO_PLAN_SUFFIX}"
    )


def is_ritardando_change(change: Mapping[str, Any]) -> bool:
    """Return whether a tempo change is a slowing that is not a feel flip."""
    ...
    if HALF_TIME_RATIO_MIN <= ratio <= HALF_TIME_RATIO_MAX:
        return False
    return True


def first_ritardando(tempo_changes: Sequence[Mapping[str, Any]] | None) -> TempoChange | None:
    """Return the earliest ritardando change, or None when none is corroborated."""
    if not isinstance(tempo_changes, Sequence) or isinstance(tempo_changes, (str, bytes)):
        return None
    for change in tempo_changes:
        if not isinstance(change, Mapping):
            continue
        if not is_ritardando_change(change):
            continue
...
def _is_named_vocal_or_bass(role: Mapping[str, Any]) -> bool:
    """Return whether a role is a named vocal or bass that may own a rit."""
    role_id = role.get("id")
    if not isinstance(role_id, str) or role_id.strip() == "":
        return False
    if role_id in NAMED_RITARDANDO_ROLE_IDS:
        return True
...
def apply_ritardando_plan(song: Mapping[str, Any], beat_times: Sequence[float] | None) -> None:
    """Attach the first corroborated ritardando plan, failing closed on bad input.
    ...
    try:
        ...
        change = first_ritardando(stability.get("tempo_changes"))
        if change is None:
            return
        copy = ritardando_plan_copy(change["from_bpm"], change["to_bpm"])
        if copy is None:
            return
        ...
        for section in sections:
            ...
            for index, role in enumerate(roles):
                ...
                stamped["ritardandoPlan"] = copy
                stamped["ritardandoPlanSource"] = "model"
                roles[index] = stamped
                return
            return
