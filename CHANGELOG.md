...
- Advance the local-audio resource policy to v3 and require the admitted decoded PCM artifact itself to be native NumPy `float32`; float16/float64 buffers now fail closed with `decoded_dtype_unsupported` even when their sample count and byte size fit, so downstream MIR cannot receive dtype-dependent numerical inputs under a nominally canonical policy.
