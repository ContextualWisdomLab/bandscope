...
- optionally invokes an in-process `track_consumer` only after both registered audio and annotation identities have passed. The callback receives that exact canonical PCM byte view, the process-owned annotation snapshot, track ID, and sample rate. It receives no workstation path;
...
The handoff regression deliberately mutates the original annotation file after its process-owned snapshot has been created and requires the consumer to observe the admitted bytes, not the mutated path content. It also requires the consumer's PCM bytes to hash to the digest emitted in the receipt.
