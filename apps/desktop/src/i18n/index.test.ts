...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-dynamics-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstDynamicsPlanOpenAction")).toBe("{at} {role} 다이내믹 열기");
      expect(t("firstDynamicsPlanBody")).toBe("{at} {section}에서 {role} 파트의 다이내믹 계획이 있습니다.");
      expect(t("firstDynamicsPlanArmed")).toBe("{at}에서 {role} 파트의 다이내믹을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
