...
| Groove Map | https://www.figma.com/design/zthWmqfNKUgJBECvv002Qk/Bandscope-Design-System-v1?node-id=19-526 | `apps/desktop/src/features/workspace/GrooveMap.tsx` | Use required `roleName`, optional `notes?: TranscriptionNote[]`, `isLoading?: boolean`, optional `entranceOnset`, and `onCancel` only when a real cancellation capability exists. Loading copy must not invent a progress percentage. Preserve scrollable region semantics, focus-visible styling, and note labels. |
