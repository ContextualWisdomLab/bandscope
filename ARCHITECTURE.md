...
- The mounted workspace copy for tonight's first confirmed chord must name the owning part when corroborated, the owned user-confirmed chord, the labeled section, and the time so the next action is obvious. Open moves to the matching rendered map section. Do not invent a confirmed chord from groove, cue, setup, simplification, overlap, range copy, model harmony, or user provenance without an owned manual override.
