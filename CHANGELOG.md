...
- 합주 워크스페이스의 읽기 전용 코드 수정 버튼이 키보드 포커스를 유지하고 `aria-disabled="true"`를 노출하며, 한·영 접근성 이름과 설명으로 편집 가능한 곡을 열어야 한다는 복구 방법을 안내하도록 개선했습니다. 포인터 `title`에도 같은 복구 문구를 제공하며 클릭은 계속 실패-폐쇄 방식으로 차단됩니다.
