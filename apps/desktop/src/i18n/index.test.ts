...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-harmonic-function next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstHarmonicFunctionOpenAction")).toBe("{at} {role} 화성 기능 위치 열기");
      expect(t("firstHarmonicFunctionBody")).toBe("{at} {section}에서 {role} 파트의 화성 기능이 있습니다.");
      expect(t("firstHarmonicFunctionArmed")).toBe("{at}에서 {role} 파트의 화성 기능을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
