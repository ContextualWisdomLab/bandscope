...
const MAX_SETUP_NOTE_CHARACTERS = 180;

/** Tonight's first setup note: the earliest labeled section and the part that owns it. */
export type FirstSetupNote = {
...
/** Format a non-negative setup-note time as m:ss for rehearsal copy. */
export function formatSetupNoteTime(totalSeconds: number): string {
...
/** Return a bounded own setup note, or null when it cannot be shown. */
function ownedSetupNote(role: unknown): string | null {
  if (!isRuntimeObject(role) || !hasOwnData(role, "setupNote")) {
    return null;
  }
  const setupNote = (role as { setupNote?: unknown }).setupNote;
  if (typeof setupNote !== "string") {
    return null;
  }
  const trimmed = setupNote.trim();
  if (trimmed.length === 0) {
    return null;
  }
  return truncateCodePoints(trimmed, MAX_SETUP_NOTE_CHARACTERS);
}
...
/** Resolve a setup note after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstSetupNote(song: RehearsalSong): FirstSetupNote | null {
  ...
  const candidates = song.sections
    ...
    .flatMap((section) => {
      const holdingRole = pickHoldingRole(
        rankedActiveRoles(section).filter((role) => ownedSetupNote(role) !== null)
      );
      if (!holdingRole) {
        return [];
      }
      const setupNote = ownedSetupNote(holdingRole);
      if (!setupNote) {
        return [];
      }
      return [
...
/** Return the first named setup note, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstSetupNote(song: RehearsalSong): FirstSetupNote | null {
  try {
    return resolveSafeFirstSetupNote(song);
  } catch {
    return null;
  }
}
