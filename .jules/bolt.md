...
## 2026-08-11 - Optimize sequential changes calculation
**Learning:** Using `zip` with list slices (e.g., `zip(lst, lst[1:])`) to compute sequential changes allocates intermediate lists and adds O(N) memory overhead in tight loops.
**Action:** Avoid building intermediate lists. Track the previous element during the initial loop iteration to achieve O(1) memory overhead and reduce allocations.
