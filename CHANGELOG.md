...
- Preserve legal `-Frag` text inside an 11-character YouTube video ID during abort cleanup: only a terminal numeric yt-dlp fragment suffix (`-FragN` or `-FragN.part`) is normalized, so IDs such as `abc-Frag123` retain their identity while their real `.part-FragN` siblings are still removed without touching same-ID nontransient files.
