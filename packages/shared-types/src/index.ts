...
/** Documented. */
function validateRehearsalDsAlFine(value: unknown, path: string): string | null {
  if (!isRecord(value)) {
    return invalidField(path);
  }
  ...
  if (!/^D\.S\. al Fine(?: [1-9])?$/u.test(value.label)) {
    return invalidField(`${path}.label`);
  }
...
function validateRehearsalSong(
  ...
  const extraKey = unexpectedKey(
    normalized,
    ["id", "title", "tempo", "dsAlFine", "sections", "exportSummary", "collaboration", "scoreAttachments"],
    ""
  );
  if (extraKey) {
    return extraKey;
  ...
  if (normalized.dsAlFine !== undefined) {
    const dsAlFineError = validateRehearsalDsAlFine(normalized.dsAlFine, "dsAlFine");
    if (dsAlFineError) {
      return dsAlFineError;
    }
  }
