...
const MAX_HINT_CHARACTERS = 180;

/** Tonight's first simpler take: the earliest named easier pass and the part that owns it. */
export type FirstSimplification = {
...
/** Format a non-negative simplification time as m:ss for rehearsal copy. */
export function formatSimplificationTime(totalSeconds: number): string {
...
function ownedSimplificationHint(role: RehearsalRole): string | null {
  ...
  const hint = role.simplification.trim();
  if (hint.length === 0) {
    return null;
  }
...
function hasRankedSimplification(role: RehearsalRole): boolean {
  return (
    hasOwnData(role, "id") &&
    ...
    ownedSimplificationHint(role) !== null
  );
}
...
/** Return ranked roles whose unique graph node is explicitly active and who name a simpler take. */
function rankedActiveSimplificationRoles(section: RehearsalSection): RehearsalRole[] {
  if (
  ...
  return section.roles.filter(
    (role) =>
      isRuntimeObject(role) &&
      hasRankedSimplification(role) &&
      !repeatedRoleIds.has(role.id) &&
      activeIds.has(role.id)
  );
}

/** Resolve a simpler take after the runtime root has passed its structural boundary checks. */
function resolveSafeFirstSimplification(song: RehearsalSong): FirstSimplification | null {
  if (
  ...
  const candidates = song.sections
    .filter(
      (section) =>
        ...
        rankedActiveSimplificationRoles(section).length > 0
    )
    .sort((left, right) => {
  ...
  const holdingRole = pickHighestPriorityRole(rankedActiveSimplificationRoles(section));
  const hint = holdingRole ? ownedSimplificationHint(holdingRole) : null;
  if (holdingRole === null || hint === null) {
    return null;
  }
...
/** Return the first named simpler take, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstSimplification(song: RehearsalSong): FirstSimplification | null {
  try {
    return resolveSafeFirstSimplification(song);
  } catch {
    return null;
  }
}
