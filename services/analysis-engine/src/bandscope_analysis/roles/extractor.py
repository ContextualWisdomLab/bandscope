...
_HIT_PLAN_PREFIX = "Land this hit with "
_HIT_PLAN_SUFFIX = "; don't drift past the downbeat."
_HIT_PLAN_BAND_TARGET = "the rest of the band"
...
class RoleExtractor:
    ...
    @staticmethod
    def _activity_hit_plan(
        role_id: str,
        roles: dict[str, RehearsalRole],
        previous_role_activity: dict[str, bool] | None,
        role_activity: dict[str, bool],
    ) -> str | None:
        """Return bounded hit guidance only for a shared simultaneous activation.

        A hit plan is emitted only when real stem activity shows this role
        becoming active with at least one other distinct source in the same
        section. Mixed-source simultaneous activation is the evidence. Heuristic
        fallback topology and first-section (no previous activity) produce no plan.
        """
        if (
        ...
        activating_role_ids = [
            ...
            if is_active and not previous_role_activity.get(candidate_id, False)
        ]
        named_source_ids = [
            candidate_id
            for candidate_id in activating_role_ids
            if candidate_id not in _OTHER_STEM_ROLE_IDS
        ]
        other_stem_activating = any(
            candidate_id in _OTHER_STEM_ROLE_IDS for candidate_id in activating_role_ids
        )
        source_count = len(named_source_ids) + (1 if other_stem_activating else 0)
        if source_count < 2:
            return None
        if source_count >= 3:
            return f"{_HIT_PLAN_PREFIX}{_HIT_PLAN_BAND_TARGET}{_HIT_PLAN_SUFFIX}"
        ...
        elif other_stem_activating and role_id not in _OTHER_STEM_ROLE_IDS:
            other_name = _OTHER_STEM_SOURCE_LABEL

        if other_name is None:
            return None
        return f"{_HIT_PLAN_PREFIX}{other_name}{_HIT_PLAN_SUFFIX}"
    ...
    def _build_activity_topology(
        ...
        for role_id, role_key in role_id_to_key.items():
            ...
            if is_active:
                role = roles[role_key]
                hit_plan = self._activity_hit_plan(
                    role_id,
                    roles,
