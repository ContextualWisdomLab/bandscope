...
/** Tonight's first named leftover last-dropout remaining return after leftover last-dropout remaining. */
export type FirstLeftoverLastDropoutRemainingReturn = {
  sectionLabel: string;
 ...
 * last-dropout remaining keep its trustworthy display name across leftover
 * sit-out, remaining leftover, leftover last-return, leftover last-dropout,
 * and leftover last-dropout remaining.
 */
 ...
 * closed so a leftover last-dropout cannot be both remaining and returning in
 * the same leftover last-dropout remaining decision.
 */
...
type PendingRemainingDropout = {
  ...
  lastReturnSectionLabel: string;
  dropoutSectionLabel: string;
  leftoverLastDropoutRemainingSectionLabel: string;
  fromSectionLabel: string;
  leftoverIds: string[];
  remainingIds: string[];
  originalSitOutIds: string[];
  lastRoleId: string;
  dropoutIds: string[];
  leftoverLastDropoutRemainingIds: string[];
  leftoverLastDropoutReturningIds: string[];
};

/**
 * Return whether the selected part belongs to this leftover last-dropout remaining return.
 *
 * A leftover last-dropout remaining return is shown only after leftover last-dropout
 * remaining whose original sit-out, leftover, remaining leftover, last leftover,
 * leftover last-dropout, or leftover last-dropout remaining cohort includes that
 * selected part, so a silent always-active part is never told to count someone in.
 */
function selectedPartBelongs(
  pending: PendingDropout | PendingRemainingDropout,
  activeRole: string | null
): boolean {
  if (!activeRole) {
    return true;
  }
  const leftoverLastDropoutRemainingIds =
    "leftoverLastDropoutRemainingIds" in pending ? pending.leftoverLastDropoutRemainingIds : [];
  return (
    ...
    leftoverLastDropoutRemainingIds.includes(activeRole)
  );
}

/**
 * Pick the first leftover last-dropout remaining return a player should honor.
 *
 ...
 * Leftover last-dropout remaining is the first later named section after that
 ...
 * own-property tacet. A leftover last-dropout remaining return is the first
 * later named section after leftover last-dropout remaining where at least one
 * leftover last-dropout remaining named part is own-property active. Leftover
 * last-dropout remaining itself, leftover last-dropout return with nobody still
 * out, leftover last-dropout, leftover last-return, remaining leftover,
 * leftover sit-out, leftover return, come-in, tacet, tutti, new dropout after
 * remaining leftover, leftover last-dropout remaining without a later remaining
 * return, or a singleton leftover last-dropout is not a leftover last-dropout
 * remaining return.
 *
 ...
 * leftover last-dropout remaining return is shown only after leftover
 * last-dropout remaining that includes that named part, or leftover
 * last-dropout remaining return of that named part.
 */
export function firstLeftoverLastDropoutRemainingReturn(
  song: RehearsalSong | unknown,
  activeRole: string | null = null
): FirstLeftoverLastDropoutRemainingReturn | null {
  ...
  for (const sectionValue of song.sections) {
    ...
    if (pendingRemainingDropout) {
      const remaining = pendingRemainingDropout;
      const remainingNodes = remaining.leftoverLastDropoutRemainingIds.map((remainingId) =>
      ...
      const returningRemaining = remainingNodes.filter((node) => node!.active);
      const stillRemaining = remainingNodes.filter((node) => node!.active === false);

      if (returningRemaining.length > 0) {
        let returning = returningRemaining[0]!;
        if (activeRole) {
        ...
        return {
          sectionLabel,
          remainingSectionLabel: remaining.leftoverLastDropoutRemainingSectionLabel,
          ...
          remainingRoleId: returning.roleId,
          remainingRoleName: namedRoles.get(returning.roleId)!,
    ...
    if (pendingDropout) {
      ...
      if (returningDropouts.length > 0 && remainingDropouts.length > 0) {
        if (activeRole && !selectedPartBelongs(dropout, activeRole)) {
          continue;
        }
        pendingRemainingDropout = {
          ...
          leftoverLastDropoutRemainingSectionLabel: sectionLabel,
          ...
          leftoverLastDropoutRemainingIds: remainingDropouts.map((node) => node!.roleId),
          leftoverLastDropoutReturningIds: returningDropouts.map((node) => node!.roleId)
        };
        pendingDropout = null;
        continue;
      }
