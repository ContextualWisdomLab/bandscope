...
- Analysis cancellation is job-specific. Queued work can terminate as typed `cancelled`; accepted cancellation is serialized against terminal result publication. On Linux/macOS the analysis child is made leader of a dedicated process group before `exec`, and cancellation/timeout/error cleanup targets that inherited group before reaping the direct child. The timed YouTube-import process now uses the same OS containment shape before spawning the allowlisted Python importer, so timeout/wait-error cleanup can terminate ordinary yt-dlp/FFmpeg descendants that retain the group before joining inherited stdout/stderr readers. This covers ordinary descendants that remain in the process group, not descendants that deliberately change group/session. Windows remains direct-child-only until a race-free Job Object boundary is implemented. The analysis runner and GUI-independent YouTube helper still implement equivalent low-level Unix process-control primitives separately; protected adoption should consolidate them into one owner rather than preserve parallel security logic. Full-length real-audio cancellation latency, inherited pipe/handle release, temp-artifact cleanup, and peak-resource return remain acceptance work.
