...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-tuning-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstTuningPlanOpenAction")).toBe("{at} {role} 튜닝 열기");
      expect(t("firstTuningPlanBody")).toBe("{at} {section}에서 {role} 파트의 튜닝 계획이 있습니다.");
      expect(t("firstTuningPlanArmed")).toBe("{at}에서 {role} 파트의 튜닝을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
