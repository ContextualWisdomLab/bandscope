//! Streaming SHA-256 used only for local playable-stem integrity comparison.
//!
...
impl Sha256State {
    fn update(&mut self, mut bytes: &[u8]) -> io::Result<()> {
        self.message_len_bytes = self
            ...
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "SHA-256 input too large"))?;
    ...
    fn finalize(mut self) -> io::Result<[u8; DIGEST_BYTES]> {
...
pub(super) fn sha256_hex_reader(mut reader: impl Read) -> io::Result<String> {
...
mod tests {
    ...
    #[test]
    fn hashes_multiple_blocks_and_interrupted_short_reads_identically() {
        let bytes = (0..131_111)
    ...
    #[test]
    fn propagates_non_interrupted_reader_failure() {
