...
enum TurnaroundPlanSourcePayload {
    Model,
    User,
}
...
pub struct RehearsalRolePayload {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    turnaround_plan: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    turnaround_plan_source: Option<TurnaroundPlanSourcePayload>,
...
pub fn project_payload_from_content(content: &str) -> Result<RehearsalSongPayload, String> {
    if let Ok(parsed) = serde_json::from_str::<RehearsalSongPayload>(content) {
        return validate_turnaround_plan_provenance(parsed);
    }
    ...
    validate_turnaround_plan_provenance(parsed)
}

fn validate_turnaround_plan_provenance(
    ...
    for section in &payload.sections {
        for role in &section.roles {
            ...
            if role
                .turnaround_plan
                .as_ref()
                .is_some_and(|turnaround_plan| {
                    turnaround_plan.trim().is_empty()
                        || turnaround_plan.contains('\n')
                        || turnaround_plan.contains('\r')
                })
            {
                return Err("Invalid project file format".to_string());
            }
            if role.turnaround_plan.is_none() && role.turnaround_plan_source.is_some() {
                return Err("Invalid project file format".to_string());
            }
            if role.turnaround_plan.is_some() && role.turnaround_plan_source.is_none() {
                return Err("Invalid project file format".to_string());
            }
        }
    }
