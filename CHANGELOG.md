...
- Name tonight's first lyric cue on the workspace and player with part, lyric, section, and start time; the workspace action arms the cue and opens the matching map section, while the player exposes a Hear action only when its owning playback surface supplies a seek callback.
