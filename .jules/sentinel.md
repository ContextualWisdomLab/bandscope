...
## 2026-09-12 - Log Injection Vulnerability in TemporalAnalyzer
**Vulnerability:** File paths (untrusted input) were injected directly into log statements using f-strings without proper sanitization/escaping in `bandscope_analysis/temporal/analyzer.py`.
**Learning:** Python f-strings in logging bypass standard logging parameterization, making the application vulnerable to log injection if the untrusted string contains newlines or control characters, which can break log structure and fake log entries.
**Prevention:** Always use parameterized string interpolation (e.g., `logger.info("msg %s", var)`) along with escaping mechanisms like `repr()` for untrusted string values (e.g., file paths, IDs).
