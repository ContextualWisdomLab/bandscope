...
- Ready `Workspace` names tonight's first playable range and the first tutti after a reduced section, scoped to the selected role when one is active, so the map enables the next rehearsal action without opening files or export paths.
