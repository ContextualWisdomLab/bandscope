...
/** Documented. */
function validateRehearsalDaCapo(value: unknown, path: string): string | null {
  if (!isRecord(value)) {
    return invalidField(path);
  }
...
function validateRehearsalSong(
  ...
  const extraKey = unexpectedKey(
    normalized,
    ["id", "title", "tempo", "daCapo", "sections", "exportSummary", "collaboration", "scoreAttachments"],
    ""
  );
  if (extraKey) {
    return extraKey;
  ...
  if (normalized.daCapo !== undefined) {
    const daCapoError = validateRehearsalDaCapo(normalized.daCapo, "daCapo");
    if (daCapoError) {
      return daCapoError;
    }
  }
