...
/** Format a non-negative dynamics-plan time as m:ss for rehearsal copy. */
export function formatDynamicsPlanTime(totalSeconds: number): string {
...
/** Return a bounded own dynamics plan, or null when it cannot be shown. */
function ownedDynamicsPlan(role: unknown): string | null {
  if (!isRuntimeObject(role) || !hasOwnData(role, "dynamicsPlan")) {
    return null;
  }
  const dynamicsPlan = (role as { dynamicsPlan?: unknown }).dynamicsPlan;
...
/** Return the first named dynamics plan, or null when untrusted runtime metadata cannot be read safely. */
export function resolveFirstDynamicsPlan(song: RehearsalSong): FirstDynamicsPlan | null {
  try {
    return resolveSafeFirstDynamicsPlan(song);
  } catch {
    return null;
  }
}
