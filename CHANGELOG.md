...
- Added a deterministic schema-v1 offline support manifest, human-readable report, and bounded local diagnostic ring buffer. Runtime diagnostics are minimized through one allowlist before retention/output; high-confidence credential-shaped token segments and Windows drive-relative path-shaped tokens fail closed; retained events require strictly increasing sequence authority; and caller snapshots cannot mutate stored evidence.
