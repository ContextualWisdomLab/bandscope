...
| First Harmonic Explanation Callout | workspace next-action pattern | `apps/desktop/src/features/workspace/FirstHarmonicExplanationCallout.tsx` | Name the owning part when an active graph node corroborates it, the owned `harmonicExplanation` copy, the labeled section start, and the time. Do not invent that copy from `groove`, cue text, `setupNote`, `simplification`, overlap warnings, range copy, `harmony.chord`, `harmony.functionLabel`, confirmed overrides, or confidence notes. Open scrolls the renderer-owned song-structure section. Keep the unavailable state guidance-only. Distinct from first-confirmed-harmony, first-ear-check, first-overlap, first-groove, and first-form-label work. |
