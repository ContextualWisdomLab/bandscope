...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("preserves every supported English section form label", () => {
      expect(translateSectionFormLabel("en", "tag")).toBe("tag");
    ...
    it("keeps Korean first-tag next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstTagOpenAction")).toBe("{at} {role} 태그 위치 열기");
      expect(t("firstTagBody")).toBe("{at} {section}에서 {role} 파트가 마지막 한 줄을 잡습니다.");
      expect(t("firstTagArmed")).toBe("{at}에서 {role} 파트와 함께 마지막 한 줄을 잡으세요. 같이 끝내세요.");
    });
  });
});
