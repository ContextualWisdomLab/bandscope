"""Regression tests for section-local accelerando role mutation."""
...
def _beats_80_to_120() -> list[float]:
    """Return beat times that speed from 80 BPM to 120 BPM around 11.25 seconds."""
...
def test_accelerando_stamp_does_not_leak_through_a_shared_role_object() -> None:
    ...
    assert "accelerandoPlan" not in earlier["roles"][0]
    assert containing["roles"][0]["accelerandoPlanSource"] == "model"


def test_accelerando_stamp_fails_closed_if_roles_stop_being_a_list() -> None:
    ...
    assert "accelerandoPlan" not in role


def test_accelerando_stamp_fails_closed_if_selected_role_identity_disappears() -> None:
    ...
    assert "accelerandoPlan" not in role
    assert "accelerandoPlan" not in replacement_role
