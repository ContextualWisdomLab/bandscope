...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-pad-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstPadPlanOpenAction")).toBe("{at} {role} 패드 열기");
      expect(t("firstPadPlanBody")).toBe("{at} {section}에서 {role} 파트의 패드 계획이 있습니다.");
      expect(t("firstPadPlanArmed")).toBe("{at}에서 {role} 파트의 패드를 맞춘 다음 합주를 시작하세요.");
    });
  });
});
