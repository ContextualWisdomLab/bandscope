...
def test_temporal_analyzer_logs_path_safely(
    ...
    """Ensure path_str is wrapped in repr() when logged to avoid log injection."""
    import librosa
    ...
    # Verify the logger was called with repr() wrapping the path
    expected_path_str = repr(str(test_wav))
    fake_logger.info.assert_any_call("Loading and decoding audio: %s", expected_path_str)
