...
## 2026-09-04 - CSV Injection Bypass via Control Characters
**Vulnerability:** A regex designed to prevent CSV formula injection (`=`, `+`, `-`, `@`) was bypassed by prepending non-printable control characters (like `\x00` or `\x1B`). Since these characters are often ignored by spreadsheet software before formula evaluation, the payload could execute successfully despite the security logic.
**Learning:** Checking for purely whitespace or printable structural characters is insufficient for security boundaries, as regex engines don't inherently block non-printable prefixes unless instructed.
**Prevention:** Always explicitly cover control characters (`\x00-\x1F`) when building regex boundary sanitization. Ensure tools like ESLint allow this intention via bypass comments (`// eslint-disable-next-line no-control-regex`).
