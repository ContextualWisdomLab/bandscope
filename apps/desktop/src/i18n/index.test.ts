...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-pickup-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstPickupPlanOpenAction")).toBe("{at} {role} 픽업 열기");
      expect(t("firstPickupPlanBody")).toBe("{at} {section}에서 {role} 파트의 픽업 계획이 있습니다.");
      expect(t("firstPickupPlanArmed")).toBe("{at}에서 {role} 파트의 픽업을 넣은 다음 합주를 시작하세요.");
    });
  });
});
