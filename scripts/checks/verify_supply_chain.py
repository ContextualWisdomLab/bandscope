...
def release_asset_allowlist_violation(path: Path) -> str:
    ...
    return f"{path}: release asset upload must use an explicit allowlist, not artifacts/*"
    ...
    def evaluate_job(job_lines: list[str], start_line: int) -> None:
        ...
        if has_run_step:
            if path is None:
                ...
                violations.append(f"{path}:{start_line or 1} -> {OSSF_PUBLISH_USES_ONLY_VIOLATION}")
    ...
    if workflow_publishes_scorecard_results(content):
        for line_number, _ in workflow_top_level_key_lines(content, {"env", "defaults"}):
            if path is None:
                ...
                violations.append(f"{path}:{line_number} -> {OSSF_PUBLISH_GLOBAL_CONFIG_VIOLATION}")
...
def _upload_step_sarif_file(step_lines: list[str], step_indent: int) -> str | None:
    with_indent: int | None = None
    for step_line in step_lines:
...
def _normalizer_output_file(command: str) -> str | None:
    try:
        tokens = shlex.split(command)
    except ValueError:
...
def _workflow_job_step_blocks(
    line_index: int, lines: list[str], step_blocks: list[tuple[int, int, list[str]]]
...
def scorecard_sarif_upload_normalization_violations(content: str) -> list[str]:
    ...
    for index, step_indent, step_lines in step_blocks:
        if "github/codeql-action/upload-sarif" not in "\n".join(
            ...
            continue
        sarif_file = _upload_step_sarif_file(step_lines, step_indent)
        ...
        job_blocks = _workflow_job_step_blocks(index, lines, step_blocks)
        ...
        normalizer_outputs = {
            ...
            if (output := _normalizer_output_file(command)) is not None
        }
    ...
    for path in workflow_paths:
        ...
        for release_step_index, release_job_content, release_command in release_steps:
            ...
            if not (has_generator_before_publish and all_release_creates_revalidated):
                violations.append(
                    f"{path}: release asset upload must use "
...
def rust_osv_exception_violations(
    ...
    for advisory_id in sorted(set(osv_ignores) - audit_ignores):
        violations.append(
            f"{osv_config}: unexpected OSV ignore for {advisory_id} "
