...
- Ready `Workspace` names tonight's first playable range and tonight's first leftover last-dropout remaining last-return tutti so the map enables the next rehearsal action without opening files or export paths.
