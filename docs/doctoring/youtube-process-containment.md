...
Hosted run `35671610296` on exact `452ddcd6a6b63dcd0d8ef7e1f0bf48a3cef6e07f` completed successfully on both `test / resource-admission / process-output / macos-15` and `test / resource-admission / process-output / windows-2025`. Each job completed the focused `Run large stdout/stderr process-output contract` step successfully. This establishes the exact-limit dual-pipe behavior for that exact source head. It does not establish Windows descendant-tree containment, real yt-dlp/FFmpeg resource return, or a later descendant's current-head GREEN status.

The earlier self-reentering unit regression still exists in `runtime_core::tests`. It is now superseded as cross-platform acceptance evidence by the isolated helper integration test, but it remains a duplicate harness with incompatible exact-limit transport semantics. It must be removed or rewritten to delegate to the isolated helper before an all-targets desktop-core test claim can be made. The hosted integration GREEN does not erase that remaining test-suite debt.
...
No hosted RED failure is claimed for `04e4d5c1…`: the regression and production repair were consecutive ordinary descendants under cancel-in-progress. For #1244, run `35498831001` is the historical namespace-coupling RED, run `35670228761` is the cross-platform libtest-overhead RED, and run `35671610296` is the isolated-helper GREEN on exact `452ddcd6...`. Any later source or owner-path movement requires fresh exact-head evidence before merge readiness is claimed.
...
| Hosted exact `a191db03…` showed that using libtest as the payload generator adds harness bytes to the same stream and crosses the real ceiling. | Exact-limit payload generation is isolated in a feature-gated helper binary; the integration test invokes the helper as the child and the workflow executes the same target on Windows and macOS. | The helper is test-only and is not evidence about yt-dlp/FFmpeg payload shape. |
| Repository-native owner workflows can bind an evidence run to `${{ github.event.pull_request.head.sha || github.sha }}` rather than GitHub's synthetic merge ref. | `resource-admission-process-output-native.yml` checks out the exact source head and runs the same integration contract on `windows-2025` and `macos-15`. | Run `35671610296` proves focused behavior only for exact `452ddcd6…`; later head movement requires new evidence. |
...
**Mitigation.** Establish containment before execution in one GUI-independent desktop-core owner, terminate the owned Unix group before joining readers, reap the directly owned child, preserve direct-child fallback, and expose no process identifier or kill primitive to the renderer. For the capture-bound acceptance test, use a feature-gated standalone helper whose only behavior is to emit the exact per-stream ceiling, then launch it through the same production `wait_for_process_output` boundary from an integration test. Bind hosted evidence to the exact source head on both Windows Server 2025 and macOS.

**Remaining risk.** Windows requires a race-free Job Object boundary. Unix descendants can intentionally escape the inherited process group. Real-audio/real-tool evidence is still required for cancellation latency, temp cleanup, pipe/handle release, and process-resource return. The legacy self-reentering unit regression remains duplicate test debt until consolidated onto the isolated-helper contract. Hosted runner success is focused behavioral evidence, not release readiness; repository/security/review gates and protected ancestry remain separate requirements.

**Test points.** Run `process_output_large_streams` with the feature-gated helper on Windows and macOS and require exact 1 MiB stdout/stderr capture; run the descendant-pipe regression on Unix; run the Tauri cancellation/terminal-contract lanes on the same final source lineage; verify timeout/error paths terminate before reader joins; remove or rewrite the legacy self-reentry unit before claiming all-targets desktop-core GREEN; later add Windows Job Object descendant/handle tests and rights-cleared full-length real-audio/tool runs.
