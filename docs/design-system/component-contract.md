...
| Section Roadmap Card | https://www.figma.com/design/zthWmqfNKUgJBECvv002Qk/Bandscope-Design-System-v1?node-id=19-402 | `apps/desktop/src/features/workspace/SectionRoadmap.tsx` | Use `song`, `activeRole`, optional `onSongUpdate`, and optional `focusedSectionId`; `id="workspace-section-{section.id}"` is the focus target for tonight's first lock-in. |
