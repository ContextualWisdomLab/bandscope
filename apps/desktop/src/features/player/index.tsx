...
import { FirstChorusCallout } from "../workspace/FirstChorusCallout";
...
/** Player surface that names tonight's first labeled chorus and delegates playback to the owning player. */
export function PlayerFeature({ title, song, onPlayFromSeconds }: PlayerFeatureProps) {
  ...
  if (!song) {
    return (
      <section style={{ padding: "24px" }}>
        <h2>{title}</h2>
        <p style={{ color: "#999" }}>{t("firstChorusNeedsSong")}</p>
      </section>
    );
  }
  ...
  return (
    <section style={{ padding: "24px" }}>
      <h2>{title}</h2>
      <FirstChorusCallout song={song} actionMode="callback-only" onHearChorus={onPlayFromSeconds} />
      <div
        style={{
          padding: "16px",
