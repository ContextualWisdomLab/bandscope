...
def matching_evidence_lines(evidence_text: str, patterns: tuple[re.Pattern[str], ...]) -> list[str]:
...
def classify_failed_check_evidence(evidence_text: str) -> dict[str, Any]:
    ...
    if upload_step_match is not None:
        ...
        if not any(
            pattern.search(evidence_text) for pattern in ARTIFACT_UPLOAD_CONFIRMATION_PATTERNS
        ):
            return unknown(
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        print("usage: classify_failed_check_evidence.py <evidence-file>", file=sys.stderr)
        return 64
