...
Current Tauri updater APIs still materialize a verified update as in-memory bytes. The repository now owns a stricter streaming byte-admission primitive, but commercial readiness requires a production network adapter that actually streams bounded response chunks into that boundary and a bounded temporary sink. Counting progress callbacks or checking the fully buffered `Vec<u8>` after download is not equivalent evidence.
...
- Bounded updater artifact streaming: `docs/traceability/updater-bounded-download.md`
