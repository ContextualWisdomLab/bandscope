...
- The mounted workspace copy for tonight's first count must name the entering part when corroborated, the owned count cue, the labeled section, and the time so the next action is obvious. Open moves to the matching rendered map section. Do not invent an entrance from lyric, transition, groove, setup, simplification, overlap, or range copy.
