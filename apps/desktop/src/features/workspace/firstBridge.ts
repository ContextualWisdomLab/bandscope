...
/** Tonight's first labeled bridge: the earliest turn and the part that carries it. */
export type FirstBridge = {
  section: RehearsalSection;
  holdingRole: RehearsalRole | null;
  atSeconds: number;
};

/** Format a non-negative bridge time as m:ss for rehearsal copy. */
export function formatBridgeTime(totalSeconds: number): string {
...
/** Return the first labeled bridge, or null when no safe turn remains. */
export function resolveFirstBridge(song: RehearsalSong): FirstBridge | null {
  if (!isRuntimeObject(song) || !isDenseRuntimeArray(song.sections)) {
    return null;
  }

  const bridgeSections = song.sections
    .filter(
      (section) =>
