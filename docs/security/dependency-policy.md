...
- `RUSTSEC-2026-0194` and `RUSTSEC-2026-0195` for `quick-xml 0.39.4` were removed by refreshing `wayland-scanner` to `0.31.11` in `apps/desktop/src-tauri/Cargo.lock`, which drops the vulnerable `quick-xml 0.39.4` entry; the Tauri/`plist` chain already resolved to patched `quick-xml 0.41.0` via `plist 1.10.0`. The cargo-audit and OSV ignore entries are retired and must not be restored; `services/analysis-engine/tests/test_quick_xml_advisory_retirement.py` rejects any future `quick-xml <0.41.0` lockfile entry or reintroduced advisory id.
