...
## 2026-07-03 - Unsanitized projectId
**Vulnerability:** `projectId` provided via the API in `bandscope_analysis` was not sanitized before validation passed, creating a potential Path Traversal vulnerability when used in file operations downstream.
**Learning:** Only explicit matches for `.` and `..`, as well as paths containing `/` and `\` should be rejected in project identifiers.
**Prevention:** Always check if untrusted IDs match `.` or `..` exactly, or contain `/` or `\` to prevent traversing directories while allowing valid identifiers like `my..id`.
