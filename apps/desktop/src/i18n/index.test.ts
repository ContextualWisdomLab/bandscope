...
describe("i18n", () => {
  ...
  describe("translateSectionFormLabel", () => {
    ...
    it("keeps Korean first-hook-plan next-action copy particle-safe", () => {
      const t = createTranslator("ko");
      expect(t("firstHookPlanOpenAction")).toBe("{at} {role} 훅 열기");
      expect(t("firstHookPlanBody")).toBe("{at} {section}에서 {role} 파트의 훅 계획이 있습니다.");
      expect(t("firstHookPlanArmed")).toBe("{at}에서 {role} 파트의 훅을 맞춘 다음 합주를 시작하세요.");
    });
  });
});
