...
/** Tonight's first labeled verse: the earliest story line and the part that carries it. */
export type FirstVerse = {
  section: RehearsalSection;
  holdingRole: RehearsalRole | null;
  atSeconds: number;
};

/** Format a non-negative verse time as m:ss for rehearsal copy. */
export function formatVerseTime(totalSeconds: number): string {
...
/** Return the first labeled verse, or null when no safe story line remains. */
export function resolveFirstVerse(song: RehearsalSong): FirstVerse | null {
  if (!isRuntimeObject(song) || !isDenseRuntimeArray(song.sections)) {
    return null;
  }

  const verseSections = song.sections
    .filter(
      (section) =>
