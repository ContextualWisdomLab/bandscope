"""Tests for corroborated fade-plan emission."""
...
_SOLO_PLAN = "Fade this part; let the next downbeat land quieter."
_PREFIX = "Fade this part with "
_SUFFIX = "; let the next downbeat land quieter."
...
def test_activity_fade_emits_solo_plan_for_a_staying_vocal_fall() -> None:
    """A staying vocal that falls 1.8× names the fade without inventing a cutoff."""
    ...
    assert all("fadePlan" not in role or role["id"] == "lead-vocal" for role in topology["active_roles"])


def test_activity_fade_names_two_named_falls_as_partners() -> None:
    """Vocal and bass falling together point at each other."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
    ...
    assert roles_by_id["lead-vocal"]["fadePlan"] == f"{_PREFIX}Bass Guitar{_SUFFIX}"
    assert roles_by_id["bass-guitar"]["fadePlan"] == f"{_PREFIX}Lead Vocal{_SUFFIX}"
    assert "fadePlan" not in roles_by_id["keys-right"]


def test_activity_fade_stays_unnamed_without_previous_activity() -> None:
    """The first section cannot be a fade."""
    extractor = RoleExtractor()
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
        "verse-1",
...
def test_activity_fade_stays_unnamed_on_heuristic_fallback() -> None:
    """Heuristic topology must not invent a fade plan."""
    extractor = RoleExtractor()
...
def test_activity_fade_stays_unnamed_for_a_density_fill() -> None:
    """A new entrance is a drop, not a fade."""
    extractor = RoleExtractor()
    ...
    topology = extractor._build_activity_topology(
        ...
        previous,
        _energy(bass=0.2, vocal=0.2),
        _energy(bass=0.5, vocal=0.0),
    )
    assert all("fadePlan" not in role for role in topology["active_roles"])


def test_activity_fade_stays_unnamed_for_a_density_drop() -> None:
    """A thinning hold is a breakdown, not a fade."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    ...
    topology = extractor._build_activity_topology(
        ...
        None,
        previous,
        _energy(bass=0.2, vocal=0.0, other=0.0),
        _energy(bass=0.5, vocal=0.5, other=0.5),
    )
    assert all("fadePlan" not in role for role in topology["active_roles"])


def test_activity_fade_stays_unnamed_when_ratio_is_too_small() -> None:
    """A small mix dip is not a corroborated fade."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
...
def test_activity_fade_stays_unnamed_when_previous_energy_is_silent() -> None:
    """Silence-to-quiet is not a fade."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
...
def test_activity_fade_stays_unnamed_when_current_energy_is_silent() -> None:
    """Loud-to-silence is a cutoff, not a fade."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
...
def test_activity_fade_stays_unnamed_for_a_swell() -> None:
    """An intensity rise is a swell, not a fade."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
...
def test_activity_fade_does_not_assign_an_other_stem_landing() -> None:
    """The shared other stem may stay in the texture but never owns the fade."""
    extractor = RoleExtractor()
    ...
    roles_by_id = {role["id"]: role for role in topology["active_roles"]}
    assert roles_by_id["lead-vocal"]["fadePlan"] == _SOLO_PLAN
    for ambiguous_role_id in ("keys-left", "keys-right", "acoustic-guitar"):
        assert "fadePlan" not in roles_by_id[ambiguous_role_id]


def test_activity_fade_keeps_shared_accompaniment_source_across_role_swap() -> None:
...
def test_extract_emits_fade_across_real_stem_boundaries() -> None:
    ...
    assert vocal_role.get("fadePlan") == _SOLO_PLAN
    assert all("fadePlan" not in role for role in result["topologies"][0]["active_roles"])


def test_activity_fade_stays_unnamed_when_energy_maps_are_missing() -> None:
    """Activity without RMS evidence cannot name a fade."""
    extractor = RoleExtractor()
    previous = _activity(bass=True, keys_right=True, vocal=True)
    current = _activity(bass=True, keys_right=True, vocal=True)
    topology = extractor._build_activity_topology(
...
def test_activity_fade_stays_unnamed_when_role_is_inactive() -> None:
    """An inactive role cannot own a fade even if RMS looks quieter."""
    extractor = RoleExtractor()
    plan = extractor._activity_fade_plan(
        "lead-vocal",
        _roles(extractor),
...
def test_extract_leaves_fade_unnamed_when_energy_detection_fails(
